...
export class PostHogTelemetryClient extends BaseTelemetryClient {
		...
		super(
			{
				...
				events: [
					...
					// TelemetryEventName.LLM_COMPLETION // hoodycode_change
				],
			},
			debug,
		)

		this.client = new PostHog(process.env.HOODYCODE_POSTHOG_API_KEY || "", {
			host: "https://us.i.posthog.com",
			disableGeoip: false, // hoodycode_change
		})
	}
	...
	private counter = 0
	private hoodycodeToken = ""

	public override async updateIdentity(hoodycodeToken: string) {
		if (hoodycodeToken === this.hoodycodeToken) {
			console.debug("HOODYTEL: Identity up-to-date")
			return
		}
		if (!hoodycodeToken) {
			console.debug("HOODYTEL: Updating identity to machine ID")
		...
		try {
			...
			if (id === this.counter) {
				...
				this.hoodycodeToken = hoodycodeToken
				console.debug("HOODYTEL: Identity updated to:", this.distinctId)
			} else {
				console.debug("HOODYTEL: Identity update ignored, newer request in progress")
			}
		} catch (error) {
			console.error("HOODYTEL: Failed to update identity", error)
			if (id === this.counter) {
				this.distinctId = vscode.env.machineId
				this.hoodycodeToken = ""
			}
