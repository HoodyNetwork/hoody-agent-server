...
export const staticAppPropertiesSchema = z.object({
	...
	platform: z.string(),
	editorName: z.string(),
	wrapped: z.boolean(), // hoodycode_change
	wrapper: z.string().nullable(), // hoodycode_change
	wrapperTitle: z.string().nullable(), // hoodycode_change
	wrapperCode: z.string().nullable(), // hoodycode_change
	wrapperVersion: z.string().nullable(), // hoodycode_change
...
export const rooCodeTelemetryEventSchema = z.discriminatedUnion("type", [
	z.object({
		type: z.enum([
			// hoodycode_change start
			TelemetryEventName.COMMIT_MSG_GENERATED, // hoodycode_change
			TelemetryEventName.INLINE_ASSIST_QUICK_TASK, // hoodycode_change
			TelemetryEventName.INLINE_ASSIST_AUTO_TASK, // hoodycode_change
			TelemetryEventName.INLINE_ASSIST_ACCEPT_SUGGESTION, // hoodycode_change
			TelemetryEventName.INLINE_ASSIST_REJECT_SUGGESTION, // hoodycode_change
			TelemetryEventName.WEBVIEW_MEMORY_USAGE, // hoodycode_change
			// hoodycode_change end
...
export interface TelemetryClient {
	...
	updateIdentity(hoodycodeToken: string): Promise<void>
	// hoodycode_change end
