...
build_extension() {
  echo "🔨 Building the Hoody Code extension..."
  pnpm -w vsix -- --out ../bin/hoody-code-$(git rev-parse --short HEAD).vsix || exit 1
  code --install-extension ../../bin/hoody-code-$(git rev-parse --short HEAD).vsix || exit 1
  cd evals
}
...
if ! code --list-extensions 2>/dev/null | grep -q "hoodycode.Hoody-Code"; then
  code --install-extension hoodycode.Hoody-Code &>/dev/null || exit 1
fi
...
echo -n "🗄️ Syncing Hoody Code evals database... "
...
current_version=$(code --list-extensions --show-versions 2>/dev/null | grep hoodycode)
read -p "💻 Do you want to build a new version of the Hoody Code extension? [currently $current_version] (y/N): " build_extension
