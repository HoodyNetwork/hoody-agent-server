...
export default defineConfig(({ mode }) => {
	...
	// hoodycode_change start - read package.json fresh every time to avoid caching issues
	const getPkg = () => {
		try {
	...
	const define: Record<string, any> = {
		...
		"process.env.PKG_OUTPUT_CHANNEL": JSON.stringify("Hoody-Code"),
	...
	if (mode === "nightly") {
		...
		define["process.env.PKG_OUTPUT_CHANNEL"] = JSON.stringify("Hoody-Code-Nightly")
	}
	...
	return {
		...
		build: {
			...
			rollupOptions: {
				external: ["vscode"], // hoodycode_change: we inadvertently import vscode into the webview: @roo/modes => src/shared/modes => ../core/prompts/sections/custom-instructions
				output: {
		...
		server: {
			host: "0.0.0.0", // hoodycode_change
			hmr: {
				// host: "localhost", hoodycode_change
				protocol: "ws",
			},
			cors: {
				origin: "*",
		...
		optimizeDeps: {
			...
			exclude: ["@vscode/codicons", "vscode-oniguruma", "shiki", "vscode" /*hoodycode_change*/],
		},
