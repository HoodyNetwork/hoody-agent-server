...
import WelcomeView from "./components/hoodycode/Welcome/WelcomeView" // hoodycode_change
import ProfileView from "./components/hoodycode/profile/ProfileView" // hoodycode_change
import McpView from "./components/mcp/McpView"
import { MarketplaceView } from "./components/marketplace/MarketplaceView"
import ModesView from "./components/modes/ModesView"
import { HumanRelayDialog } from "./components/human-relay/HumanRelayDialog"
import BottomControls from "./components/hoodycode/BottomControls" // hoodycode_change
import { MemoryService } from "./services/MemoryService" // hoodycode_change
import { CheckpointRestoreDialog } from "./components/chat/CheckpointRestoreDialog"
import { DeleteMessageDialog, EditMessageDialog } from "./components/chat/MessageModificationConfirmationDialog"
import ErrorBoundary from "./components/ErrorBoundary"
// import { AccountView } from "./components/account/AccountView" // hoodycode_change: we have our own profile view
// import { CloudView } from "./components/cloud/CloudView" // hoodycode_change: not rendering this
import { useAddNonInteractiveClickListener } from "./components/ui/hooks/useNonInteractiveClick"
import { TooltipProvider } from "./components/ui/tooltip"
import { STANDARD_TOOLTIP_DELAY } from "./components/ui/standard-tooltip"
import { useHoodyIdentity } from "./utils/hoodycode/useHoodyIdentity"

type Tab = "settings" | "history" | "mcp" | "modes" | "chat" | "marketplace" | "account" | "cloud" | "profile" // hoodycode_change: add "profile"
...
const tabsByMessageAction: Partial<Record<NonNullable<ExtensionMessage["action"]>, Tab>> = {
	...
	// cloudButtonClicked: "cloud", // hoodycode_change: no cloud
}
...
const App = () => {
	const {
		...
		renderContext,
		mdmCompliant,
		apiConfiguration, // hoodycode_change
	} = useExtensionState()
	...
	const chatViewRef = useRef<ChatViewRef & { focusInput: () => void }>(null) // hoodycode_change
	...
	const telemetryDistinctId = useHoodyIdentity(apiConfiguration?.hoodycodeToken ?? "", machineId ?? "")
	useEffect(() => {
		if (didHydrateState) {
	...
	return showWelcome ? (
		...
		<>
			...
			{tab === "settings" && (
				<SettingsView ref={settingsRef} onDone={() => switchTab("chat")} targetSection={currentSection} /> // hoodycode_change
			)}
			{/* hoodycode_change: add profileview */}
			{tab === "profile" && <ProfileView onDone={() => switchTab("chat")} />}
			{tab === "marketplace" && (
				<MarketplaceView
					stateManager={marketplaceStateManager}
					onDone={() => switchTab("chat")}
					// hoodycode_change: targetTab="mode"
					targetTab="mode"
				/>
			)}
			{/* hoodycode_change: no cloud view */}
			{/* {tab === "cloud" && (
				<CloudView
			...
			{/* hoodycode_change: we have our own profile view */}
			{/* {tab === "account" && (
