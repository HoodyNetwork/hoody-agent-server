 ...
 */
// hoodycode_change: unused campaign param
export function buildDocLink(path: string, _campaign: string): string {
	// Remove any leading slash from path
	const cleanPath = path
		.replace(/^\//, "")
		.replace("troubleshooting/shell-integration/", "features/shell-integration") // hoodycode_change
	const [basePath, hash] = cleanPath.split("#")
	const baseUrl = `https://hoody.com/docs/${basePath}`
