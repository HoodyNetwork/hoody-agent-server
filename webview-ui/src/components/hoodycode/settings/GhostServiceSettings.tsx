...
export const GhostServiceSettingsView = ({
	...
	const { enableAutoTrigger, autoTriggerDelay, enableQuickInlineTaskKeybinding, enableSmartInlineTaskKeybinding } =
		ghostServiceSettings || {}
	const keybindings = useKeybindings(["hoody-code.ghost.promptCodeSuggestion", "hoody-code.ghost.generateSuggestions"])
		...
		<div className={cn("flex flex-col", className)} {...props}>
			<SectionHeader>
				<div className="flex items-center gap-2">
					<Bot className="w-4" />
					<div>{t("hoodycode:ghost.title")}</div>
				</div>
			</SectionHeader>
				...
				<div className="flex flex-col gap-3">
					<div className="flex flex-col gap-1">
						<div className="flex items-center gap-2 font-bold">
							<Zap className="w-4" />
							<div>{t("hoodycode:ghost.settings.triggers")}</div>
						</div>
					</div>

					<div className="flex flex-col gap-1">
						<ControlledCheckbox checked={enableAutoTrigger || false} onChange={onEnableAutoTriggerChange}>
							<span className="font-medium">{t("hoodycode:ghost.settings.enableAutoTrigger.label")}</span>
						</ControlledCheckbox>
						<div className="text-vscode-descriptionForeground text-sm mt-1">
							<Trans i18nKey="hoodycode:ghost.settings.enableAutoTrigger.description" />
						</div>
					</div>
					...
					{enableAutoTrigger && (
						<div className="flex flex-col gap-1">
							<label className="block font-medium text-sm">
								{t("hoodycode:ghost.settings.autoTriggerDelay.label")}
							</label>
							...
							<div className="text-vscode-descriptionForeground text-xs mt-1">
								<Trans i18nKey="hoodycode:ghost.settings.autoTriggerDelay.description" />
							</div>
						</div>
					)}
					...
					<div className="flex flex-col gap-1">
						<ControlledCheckbox
							...
							<span className="font-medium">
								{t("hoodycode:ghost.settings.enableQuickInlineTaskKeybinding.label", {
									keybinding: keybindings["hoody-code.ghost.promptCodeSuggestion"],
								})}
							</span>
						</ControlledCheckbox>
						<div className="text-vscode-descriptionForeground text-sm mt-1">
							<Trans
								i18nKey="hoodycode:ghost.settings.enableQuickInlineTaskKeybinding.description"
								components={{
									DocsLink: (
										<a
											href="#"
											onClick={() =>
												openGlobalKeybindings("hoody-code.ghost.promptCodeSuggestion")
											}
					...
					<div className="flex flex-col gap-1">
						<ControlledCheckbox
							...
							<span className="font-medium">
								{t("hoodycode:ghost.settings.enableSmartInlineTaskKeybinding.label", {
									keybinding: keybindings["hoody-code.ghost.generateSuggestions"],
								})}
							</span>
						</ControlledCheckbox>
						<div className="text-vscode-descriptionForeground text-sm mt-1">
							<Trans
								i18nKey="hoodycode:ghost.settings.enableSmartInlineTaskKeybinding.description"
								values={{ keybinding: keybindings["hoody-code.ghost.generateSuggestions"] }}
								components={{
									DocsLink: (
										<a
											href="#"
											onClick={() => openGlobalKeybindings("hoody-code.ghost.generateSuggestions")}
											className="text-[var(--vscode-list-highlightForeground)] hover:underline cursor-pointer"></a>
									),
								}}
							/>
