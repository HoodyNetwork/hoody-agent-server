...
import { getExtension } from "@/utils/hoodycode/path-webview"
import { allowedExtensions } from "@roo/hoodycode/rules"
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		if (filename.trim()) {
			...
			if (extension !== "" && !allowedExtensions.includes(extension)) {
				setError(t("hoodycode:rules.validation.invalidFileExtension"))
				return
			}
				...
				{isExpanded ? (
					<form onSubmit={handleSubmit} className="flex flex-1 items-center">
						<input
							...
							placeholder={
								ruleType === "workflow"
									? t("hoodycode:rules.placeholders.workflowName")
									: t("hoodycode:rules.placeholders.ruleName")
							}
							value={filename}
					...
					<>
						<span className="flex-1 text-[var(--vscode-descriptionForeground)] bg-[var(--vscode-input-background)] italic text-xs">
							{ruleType === "workflow"
								? t("hoodycode:rules.newFile.newWorkflowFile")
								: t("hoodycode:rules.newFile.newRuleFile")}
						</span>
						<div className="flex items-center ml-2 space-x-2">
							<VSCodeButton
								appearance="icon"
