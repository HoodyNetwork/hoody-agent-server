...
const HoodyRulesToggleModal: React.FC = () => {
					...
					<Tooltip open={isVisible ? false : undefined}>
						<TooltipTrigger asChild>
							<BottomButton
								...
								ariaLabel={t("hoodycode:rules.ariaLabel")}
								onClick={() => setIsVisible(!isVisible)}
							/>
						</TooltipTrigger>
						<TooltipContent>{t("hoodycode:rules.tooltip")}</TooltipContent>
					</Tooltip>
				</TooltipProvider>
			</div>
			...
			{isVisible && (
				<div
					...
					<div
						...
						<div
							...
							<StyledTabButton $isActive={currentView === "rule"} onClick={() => setCurrentView("rule")}>
								{t("hoodycode:rules.tabs.rules")}
							</StyledTabButton>
							<StyledTabButton
								$isActive={currentView === "workflow"}
								onClick={() => setCurrentView("workflow")}>
								{t("hoodycode:rules.tabs.workflows")}
							</StyledTabButton>
						</div>
					</div>
					...
					<div className="text-xs text-[var(--vscode-descriptionForeground)] mb-4">
						{currentView === "rule" ? (
							<DescriptionWithLink
								href="https://hoody.com/docs/advanced-usage/custom-rules"
								linkText={t("hoodycode:docs")}>
								{t("hoodycode:rules.description.rules")}
							</DescriptionWithLink>
						) : (
							<DescriptionWithLink
								href="https://hoody.com/docs/features/slash-commands/workflows"
								linkText={t("hoodycode:docs")}>
								{t("hoodycode:rules.description.workflows")}{" "}
								<span className="text-[var(--vscode-foreground)] font-bold">/workflow-name</span>{" "}
								{t("hoodycode:rules.description.workflowsInChat")}
							</DescriptionWithLink>
						)}
					</div>
...
export default HoodyRulesToggleModal
