...
import { isAlphaPeriodEndedError, isModelNotAllowedForTeamError } from "@roo/hoodycode/errorUtils"
...
function warningText(innerMessage?: InnerMessage) {
	...
	if (isAlphaPeriodEndedError(innerMessage?.error)) {
		return i18next.t("hoodycode:invalidModel.alphaPeriodEnded", { model: unavailableModel })
	}
	if (isModelNotAllowedForTeamError(innerMessage?.error)) {
		return i18next.t("hoodycode:invalidModel.notAllowedForTeam", { model: unavailableModel })
	}
	return i18next.t("hoodycode:invalidModel.modelUnavailable", { model: unavailableModel })
}
				...
				<>
					<VSCodeButton
						...
						{canChangeToDefaultModel
							? t("hoodycode:invalidModel.continueWith", {
									model: providerModels[defaultModelId]?.displayName ?? defaultModelId,
								})
							: t("hoodycode:invalidModel.continue")}
					</VSCodeButton>
