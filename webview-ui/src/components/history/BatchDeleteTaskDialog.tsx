...
import { useTaskWithId } from "@/hoodycode/hooks/useTaskHistory"
...
export const BatchDeleteTaskDialog = ({ taskIds, ...props }: BatchDeleteTaskDialogProps) => {
	...
	const { data: tasks } = useTaskWithId(taskIds) // hoodycode_change
	const { onOpenChange } = props

	const favoritedTasks = tasks?.filter((task) => taskIds.includes(task.id) && task.isFavorited) ?? [] // hoodycode_change
	const hasFavoritedTasks = favoritedTasks.length > 0 // hoodycode_change
