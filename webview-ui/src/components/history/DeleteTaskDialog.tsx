...
export const DeleteTaskDialog = ({ taskId, ...props }: DeleteTaskDialogProps) => {
	...
	const { data: tasks } = useTaskWithId([taskId]) // hoodycode_change

	const { onOpenChange } = props

	const task = tasks?.find((t) => t.id === taskId) // hoodycode_change
	const isFavorited = task?.isFavorited // hoodycode_change
