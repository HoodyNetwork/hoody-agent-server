...
vi.mock("@/hoodycode/hooks/useTaskHistory")
...
function hoodyCodeSetUpUseTaskHistoryMock(useTaskSearchReturnValue: Partial<ReturnType<typeof useTaskSearch>>) {
...
const mockHoodyCodeTaskHistoryVersion = 0
...
describe("HistoryPreview", () => {
	...
	it("renders nothing when no tasks are available", () => {
		hoodyCodeSetUpUseTaskHistoryMock({
			tasks: [],
			searchQuery: "",
		...
		const { container } = render(<HistoryPreview taskHistoryVersion={mockHoodyCodeTaskHistoryVersion} />)
	...
	it("renders up to 3 tasks when tasks are available", () => {
		...
		render(<HistoryPreview taskHistoryVersion={mockHoodyCodeTaskHistoryVersion} />)
