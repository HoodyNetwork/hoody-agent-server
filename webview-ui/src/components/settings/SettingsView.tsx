...
import {
	...
	Globe,
	Info,
	Server, // hoodycode_change
	Bot, // hoodycode_change
	MessageSquare,
	Monitor,
	LucideIcon,
	// SquareSlash, // hoodycode_change
	// Glasses, // hoodycode_change
} from "lucide-react"
...
import { DisplaySettings } from "./DisplaySettings" // hoodycode_change
...
import McpView from "../hoodycodeMcp/McpView" // hoodycode_change
import deepEqual from "fast-deep-equal" // hoodycode_change
import { GhostServiceSettingsView } from "../hoodycode/settings/GhostServiceSettings" // hoodycode_change
...
export const settingsTabTrigger =
	"whitespace-nowrap overflow-hidden min-w-0 h-12 px-4 py-3 box-border flex items-center border-l-2 border-transparent text-vscode-foreground opacity-70 hover:bg-vscode-list-hoverBackground data-[compact=true]:w-12 data-[compact=true]:p-4 cursor-pointer" // hoodycode_change add cursor-pointer
export const settingsTabTriggerActive =
	"opacity-100 border-vscode-focusBorder bg-vscode-list-activeSelectionBackground hover:bg-vscode-list-activeSelectionBackground cursor-default" // hoodycode_change add hover:bg-* and cursor-default
...
const sectionNames = [
	...
	"browser",
	"checkpoints",
	"ghost", // hoodycode_change
	"display", // hoodycode_change
	"notifications",
	"contextManagement",
	"terminal",
	"prompts",
...
type SectionName = (typeof sectionNames)[number] // hoodycode_change
...
const SettingsView = forwardRef<SettingsViewRef, SettingsViewProps>(({ onDone, targetSection }, ref) => {
	...
	const {
		...
		listApiConfigMeta,
		uriScheme,
		hoodyCodeWrapperProperties, // hoodycode_change
		settingsImportedAt,
	} = extensionState
	...
	const {
		...
		ghostServiceSettings, // hoodycode_change
	...
	const { hoodycodeToken, openRouterApiKey, glamaApiKey, requestyApiKey } = extensionState.apiConfiguration ?? {}
	useEffect(() => {
	...
	}, [hoodycodeToken, openRouterApiKey, glamaApiKey, requestyApiKey])
	...
	const setHoodyCodeImageApiKey = useCallback((apiKey: string) => {
		setCachedState((prevState) => {
			setChangeDetected(true)
			return { ...prevState, hoodyCodeImageApiKey: apiKey }
		})
	}, [])
	...
	const handleSubmit = () => {
		if (isSettingValid) {
			...
			vscode.postMessage({ type: "terminalCommandApiConfigId", text: terminalCommandApiConfigId || "" }) // hoodycode_change
			...
			vscode.postMessage({ type: "showAutoApproveMenu", bool: showAutoApproveMenu }) // hoodycode_change
			...
			vscode.postMessage({ type: "allowVeryLargeReads", bool: allowVeryLargeReads }) // hoodycode_change
			...
			vscode.postMessage({ type: "showTaskTimeline", bool: showTaskTimeline }) // hoodycode_change
			vscode.postMessage({ type: "showTimestamps", bool: showTimestamps }) // hoodycode_change
			vscode.postMessage({ type: "hideCostBelowThreshold", value: hideCostBelowThreshold }) // hoodycode_change
			...
			vscode.postMessage({ type: "systemNotificationsEnabled", bool: systemNotificationsEnabled }) // hoodycode_change
			vscode.postMessage({ type: "ghostServiceSettings", values: ghostServiceSettings }) // hoodycode_change
			vscode.postMessage({ type: "morphApiKey", text: morphApiKey }) // hoodycode_change
			vscode.postMessage({ type: "fastApplyModel", text: fastApplyModel }) // hoodycode_change: Fast Apply model selection
			vscode.postMessage({ type: "openRouterImageApiKey", text: openRouterImageApiKey })
			vscode.postMessage({ type: "hoodyCodeImageApiKey", text: hoodyCodeImageApiKey })
			vscode.postMessage({
	...
	const sections: { id: SectionName; icon: LucideIcon }[] = useMemo(
		() => [
			...
			// { id: "slashCommands", icon: SquareSlash }, // hoodycode_change: needs work to be re-introduced
			{ id: "browser", icon: SquareMousePointer },
			{ id: "checkpoints", icon: GitBranch },
			{ id: "display", icon: Monitor }, // hoodycode_change
			...(hoodyCodeWrapperProperties?.hoodyCodeWrapped ? [] : [{ id: "ghost" as const, icon: Bot }]), // hoodycode_change
			{ id: "notifications", icon: Bell },
			{ id: "contextManagement", icon: Database },
			{ id: "terminal", icon: SquareTerminal },
			{ id: "prompts", icon: MessageSquare },
			// { id: "ui", icon: Glasses }, // hoodycode_change: we have our own display section
			{ id: "experimental", icon: FlaskConical },
			{ id: "language", icon: Globe },
			{ id: "mcp", icon: Server },
			{ id: "about", icon: Info },
		],
		[hoodyCodeWrapperProperties?.hoodyCodeWrapped], // hoodycode_change
	)
	...
	}, [targetSection]) // hoodycode_change
			...
			<div ref={containerRef} className={cn(settingsTabsContainer, isCompactMode && "narrow")}>
							...
							<TabTrigger
								...
								<div className={cn("flex items-center gap-2", isCompactMode && "justify-center")}>
									...
									<span className="tab-label">
										{id === "mcp"
											? t(`hoodycode:settings.sections.mcp`)
											: id === "ghost"
												? t(`hoodycode:ghost.title`)
				...
				<TabContent ref={contentRef} className="p-0 flex-1 overflow-auto">
					...
					{activeTab === "autoApprove" && (
						<AutoApproveSettings
							showAutoApproveMenu={showAutoApproveMenu} // hoodycode_change
					...
					{/* hoodycode_change start display section */}
					{activeTab === "display" && (
						<DisplaySettings
							reasoningBlockCollapsed={reasoningBlockCollapsed ?? true}
							showTaskTimeline={showTaskTimeline}
							showTimestamps={cachedState.showTimestamps} // hoodycode_change
					...
					{/* hoodycode_change end display section */}
					...
					{activeTab === "contextManagement" && (
						<ContextManagementSettings
							...
							allowVeryLargeReads={allowVeryLargeReads /* hoodycode_change */}
					...
					{activeTab === "terminal" && (
						<TerminalSettings
							...
							terminalCommandApiConfigId={terminalCommandApiConfigId} // hoodycode_change
							setCachedStateField={setCachedStateField}
						/>
					)}
					...
					{activeTab === "experimental" && (
						<ExperimentalSettings
							...
							hoodyCodeImageApiKey={hoodyCodeImageApiKey}
							openRouterImageGenerationSelectedModel={
								openRouterImageGenerationSelectedModel as string | undefined
							}
							setOpenRouterImageApiKey={setOpenRouterImageApiKey}
							setHoodyCodeImageApiKey={setHoodyCodeImageApiKey}
							setImageGenerationSelectedModel={setImageGenerationSelectedModel}
							currentProfileHoodycodeToken={apiConfiguration.hoodycodeToken}
						/>
					)}
