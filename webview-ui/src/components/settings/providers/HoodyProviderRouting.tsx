...
type ProviderSortValue = typeof DEFAULT_VALUE_KEY | "price" | "throughput" | "latency"

type ProviderPreference =
	| {
			type: ProviderSortValue
	  }
	| {
...
export const HoodyProviderRoutingManagedByOrganization = () => {
	const { t } = useAppTranslation()
	return (
		<div className="flex flex-col gap-1">
			<div className="flex justify-between items-center">
				<label className="block font-medium mb-1">
					{t("hoodycode:settings.provider.providerRouting.title")}
				</label>
			</div>
			<div className="text-sm text-vscode-descriptionForeground">
				{t("hoodycode:settings.provider.providerRouting.managedByOrganization")}
			</div>
		</div>
	)
}

export const HoodyProviderRouting = ({ apiConfiguration, setApiConfigurationField, hoodycodeDefaultModel }: Props) => {
	const { t } = useAppTranslation()
	const providers = Object.values(useModelProviders(hoodycodeDefaultModel, apiConfiguration).data ?? {})
			...
			<Select value={JSON.stringify(getProviderPreference(apiConfiguration))} onValueChange={onValueChange}>
				...
				<SelectContent>
					<ProviderSelectItem value={{ type: DEFAULT_VALUE_KEY }}>
						{t("hoodycode:settings.provider.providerRouting.sorting.default")}
					</ProviderSelectItem>
					<ProviderSelectItem value={{ type: openRouterProviderSortSchema.Values.price }}>
						{t("hoodycode:settings.provider.providerRouting.sorting.price")}
					</ProviderSelectItem>
					<ProviderSelectItem value={{ type: openRouterProviderSortSchema.Values.throughput }}>
						{t("hoodycode:settings.provider.providerRouting.sorting.throughput")}
					</ProviderSelectItem>
					<ProviderSelectItem value={{ type: openRouterProviderSortSchema.Values.latency }}>
						{t("hoodycode:settings.provider.providerRouting.sorting.latency")}
					</ProviderSelectItem>
					<SelectSeparator />
			...
			<Select
				...
				<SelectContent>
					<SelectItem value={DEFAULT_VALUE_KEY}>
						{apiConfiguration.apiProvider === "hoodycode"
							? t("hoodycode:settings.provider.providerRouting.dataCollection.allowFree")
							: t("hoodycode:settings.provider.providerRouting.dataCollection.default")}
					</SelectItem>
					<SelectItem value={openRouterProviderDataCollectionSchema.Values.allow}>
						{t("hoodycode:settings.provider.providerRouting.dataCollection.allow")}
					</SelectItem>
					<SelectItem value={openRouterProviderDataCollectionSchema.Values.deny}>
						{t("hoodycode:settings.provider.providerRouting.dataCollection.deny")}
					</SelectItem>
					<SelectItem value={ZERO_DATA_RETENTION_KEY}>
						{t("hoodycode:settings.provider.providerRouting.dataCollection.zdr")}
					</SelectItem>
				</SelectContent>
			</Select>
		</div>
