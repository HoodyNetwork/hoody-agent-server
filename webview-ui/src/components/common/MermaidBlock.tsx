...
import { MermaidSyntaxFixer } from "@src/services/mermaidSyntaxFixer" // hoodycode_change
import CodeBlock from "../hoodycode/common/CodeBlock" // hoodycode_change
import { MermaidButton } from "@/components/common/MermaidButton"
import { MermaidFixButton } from "@/components/common/MermaidFixButton" // hoodycode_change
...
// hoodycode_change next line rename to originalCode to keep the difference with Roo smaller
export default function MermaidBlock({ code: originalCode }: MermaidBlockProps) {
	...
	useEffect(() => {
		...
		setIsFixing(false)
		// hoodycode_change end
	}, [originalCode]) // hoodycode_change originalCode instead of code
	...
	useDebounceEffect(
		...
		500, // Delay 500ms
		[code, isFixing, originalCode, t], // Dependencies for scheduling // hoodycode_change added isFixing, originalCode and t
	)
	...
	return (
		<MermaidBlockContainer>
			{isLoading && (
				<LoadingMessage>
					{isFixing /* hoodycode_change */ ? t("common:mermaid.fixing_syntax") : t("common:mermaid.loading")}
				</LoadingMessage>
			)}
			...
			{error ? (
				...
				<MermaidButton containerRef={containerRef} code={code} isLoading={isLoading} svgToPng={svgToPng}>
					{/* hoodycode_change start switched from ref to dangerouslySetInnerHTML */}
					<SvgContainer
						onClick={handleClick}
						ref={containerRef}
						$isLoading={isLoading}
