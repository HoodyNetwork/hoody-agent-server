...
export const MarketplaceItemCard: React.FC<MarketplaceItemCardProps> = ({ item, filters, setFilters, installed }) => {
	...
	const [installModalVersion, setInstallModalVersion] = useState(0) // hoodycode_change
	...
	const handleInstallClick = () => {
		...
		setInstallModalVersion((prev) => prev + 1) // hoodycode_change
	...
	return (
		<>
			...
			<MarketplaceInstallModal
				key={`install-modal-${item.id}-${installModalVersion}` /* hoodycode_change */}
				item={item}
