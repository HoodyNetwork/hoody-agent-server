...
import { DropdownOptionType, Button, StandardTooltip } from "@/components/ui" // hoodycode_change

import Thumbnails from "../common/Thumbnails"
import { ModeSelector } from "./ModeSelector"
import HoodyModeSelector from "../hoodycode/HoodyModeSelector"
import { HoodyProfileSelector } from "../hoodycode/chat/HoodyProfileSelector" // hoodycode_change
import { MAX_IMAGES_PER_MESSAGE } from "./ChatView"
import ContextMenu from "./ContextMenu"
import { ImageWarningBanner } from "./ImageWarningBanner" // hoodycode_change
import {
	VolumeX,
	Pin,
	Check,
	// Image, // hoodycode_change
	WandSparkles,
	SendHorizontal,
	Paperclip, // hoodycode_change
} from "lucide-react"
...
// hoodycode_change start: pull slash commands from Cline
...
export const ChatTextArea = forwardRef<HTMLTextAreaElement, ChatTextAreaProps>(
	(
		...
		const {
			...
			localWorkflows, // hoodycode_change
			globalWorkflows, // hoodycode_change
		...
		// hoodycode_change begin: remove button from chat when it gets to small
		...
		const [imageWarning, setImageWarning] = useState<string | null>(null) // hoodycode_change
		...
		// hoodycode_change start: Image warning handlers
		...
		// hoodycode_change end: Image warning handlers

		// hoodycode_change start: Clear images if unsupported
		...
		useEffect(() => {
			if (!prevShouldDisableImages.current && shouldDisableImages && selectedImages.length > 0) {
				setSelectedImages([])
				showImageWarning("hoodycode:imageWarnings.imagesRemovedNoSupport")
			}
		...
		// hoodycode_change end: Clear images if unsupported
		...
		const handleInputChange = useCallback(
			(e: React.ChangeEvent<HTMLTextAreaElement>) => {
				...
				if (showMenu) {
					// hoodycode_change start - check lastAtIndex before handling slash commands
					const lastAtIndex = newValue.lastIndexOf("@", newCursorPosition - 1)

					// if (newValue.startsWith("/")) { ⚠️ hoodycode_change added lastAtIndex check
		...
		const handleBlur = useCallback(() => {
			...
			if (!isMouseDownOnMenu) {
				setShowContextMenu(false)
				setShowSlashCommandsMenu(false) // hoodycode_change: pull slash commands from Cline
			}
		...
		const handlePaste = useCallback(
			async (e: React.ClipboardEvent) => {
				...
				// hoodycode_change start: Image validation with warning messages
				if (imageItems.length > 0) {
					e.preventDefault()

					if (shouldDisableImages) {
						showImageWarning(`hoodycode:imageWarnings.modelNoImageSupport`)
						return
					}
					if (selectedImages.length >= MAX_IMAGES_PER_MESSAGE) {
						showImageWarning(`hoodycode:imageWarnings.maxImagesReached`)
						return
					}
					// hoodycode_change end: Image validation with warning messages
			...
			[
				...
				inputValue,
				t,
				selectedImages.length, // hoodycode_change - added selectedImages.length
				showImageWarning, // hoodycode_change - added showImageWarning
			],
		)
		...
		const handleDrop = useCallback(
			async (e: React.DragEvent<HTMLDivElement>) => {
				...
				if (files.length > 0) {
					...
					// hoodycode_change start: Image validation with warning messages for drag and drop
					if (imageFiles.length > 0) {
						...
						// hoodycode_change end: Image validation with warning messages for drag and drop
		...
		const renderNonEditModeControls = () => (
			// hoodycode_change move thumbnails to bottom
			...
			<div
				...
				<div className={cn("flex", "items-center", "gap-1", "min-w-0")}>
					<div className="shrink-0">
						{/* hoodycode_change start: HoodyModeSelector instead of ModeSelector */}
						<HoodyModeSelector
							value={mode}
							onChange={setMode}
							modeShortcutText={modeShortcutText}
				...
				{/* hoodycode_change: hidden on small containerWidth
		...
		const renderTextAreaSection = () => (
			<div
				...
				<DynamicTextArea
					...
					// hoodycode_change: combine placeholderText and placeholderBottomText here
					...
					className={cn(
						...
						"scrollbar-none",
						"scrollbar-hide",
						"pb-16", // hoodycode_change: Increased padding to prevent overlap with control bar
					)}
					onScroll={() => updateHighlights()}
				/>
				{/* hoodycode_change {Transparent overlay at bottom of textArea to avoid text overlap } */}
				<div
				...
				{/* hoodycode_change: position tweaked */}
				...
				{/* hoodycode_change: position tweaked, rtl support */}
				...
				{!inputValue && (
					<div
						...
						{/* hoodycode_change {placeholderBottomText} */}
					</div>
				)}
			</div>
		)
			...
			<div
				...
				<div className="relative">
					<div
						...
						{/* hoodycode_change start: ImageWarningBanner integration */}
						<ImageWarningBanner
							messageKey={imageWarning ?? ""}
							onDismiss={dismissImageWarning}
							isVisible={!!imageWarning}
						/>
						{/* hoodycode_change end: ImageWarningBanner integration */}
						{/* hoodycode_change start: pull slash commands from Cline */}
						{showSlashCommandsMenu && (
						...
						{/* hoodycode_change end: pull slash commands from Cline */}
						{showContextMenu && (
							<div
						...
						{/* hoodycode_change: renderNonEditModeControls moved */}
				...
				{selectedImages.length > 0 && (
					<Thumbnails
						...
						style={{
							left: "16px",
							zIndex: 2,
							marginTop: "14px", // hoodycode_change
							marginBottom: 0,
						}}
					/>
				)}
