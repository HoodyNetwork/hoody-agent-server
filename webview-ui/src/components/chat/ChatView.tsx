...
// import RooHero from "@src/components/welcome/RooHero" // hoodycode_change: unused
// import RooTips from "@src/components/welcome/RooTips" // hoodycode_change: unused
import { StandardTooltip } from "@src/components/ui"
import { useAutoApprovalState } from "@src/hooks/useAutoApprovalState"
import { useAutoApprovalToggles } from "@src/hooks/useAutoApprovalToggles"
// import { CloudUpsellDialog } from "@src/components/cloud/CloudUpsellDialog" // hoodycode_change: unused

import TelemetryBanner from "../common/TelemetryBanner" // hoodycode_change: deactivated for now
// import VersionIndicator from "../common/VersionIndicator" // hoodycode_change: unused
import { OrganizationSelector } from "../hoodycode/common/OrganizationSelector"
// import { useTaskSearch } from "../history/useTaskSearch" // hoodycode_change: unused
import HistoryPreview from "../history/HistoryPreview"
import Announcement from "./Announcement"
import BrowserSessionRow from "./BrowserSessionRow"
import ChatRow from "./ChatRow"
import { ChatTextArea } from "./ChatTextArea"
// import TaskHeader from "./TaskHeader"// hoodycode_change
import HoodyTaskHeader from "../hoodycode/HoodyTaskHeader" // hoodycode_change
import AutoApproveMenu from "./AutoApproveMenu"
import BottomControls from "../hoodycode/BottomControls" // hoodycode_change
import SystemPromptWarning from "./SystemPromptWarning"
import { showSystemNotification } from "@/hoodycode/helpers" // hoodycode_change
// import ProfileViolationWarning from "./ProfileViolationWarning" hoodycode_change: unused
import { CheckpointWarning } from "./CheckpointWarning"
import { IdeaSuggestionsBox } from "../hoodycode/chat/IdeaSuggestionsBox" // hoodycode_change
import { HoodycodeNotifications } from "../hoodycode/HoodycodeNotifications" // hoodycode_change
import { QueuedMessages } from "./QueuedMessages"
import { buildDocLink } from "@/utils/docLinks"
// import DismissibleUpsell from "../common/DismissibleUpsell" // hoodycode_change: unused
// import { useCloudUpsell } from "@src/hooks/useCloudUpsell" // hoodycode_change: unused
// import { Cloud } from "lucide-react" // hoodycode_change: unused
...
export interface ChatViewRef {
	acceptInput: () => void
	focusInput: () => void // hoodycode_change
}
...
const ChatViewComponent: React.ForwardRefRenderFunction<ChatViewRef, ChatViewProps> = (
	...
	const {
		...
		currentTaskItem,
		currentTaskTodos,
		taskHistoryFullLength, // hoodycode_change
		taskHistoryVersion, // hoodycode_change
		apiConfiguration,
		...
		enableCheckpoints, // hoodycode_change
		alwaysAllowSubtasks,
		...
		soundEnabled,
		soundVolume,
		// cloudIsAuthenticated, // hoodycode_change
		messageQueue = [],
	} = useExtensionState()
	...
	// const { tasks } = useTaskSearch() // hoodycode_change
	...
	const [forkCountsByTs, setForkCountsByTs] = useState<Record<number, number>>({})
					...
					switch (lastMessage.ask) {
						...
						case "tool":
							if (!isAutoApproved(lastMessage) && !isPartial) {
								playSound("notification")
								showSystemNotification(t("hoodycode:notifications.toolRequest")) // hoodycode_change
							}
							setSendingDisabled(isPartial)
							setClineAsk("tool")
						...
						case "browser_action_launch":
							if (!isAutoApproved(lastMessage) && !isPartial) {
								playSound("notification")
								showSystemNotification(t("hoodycode:notifications.browserAction")) // hoodycode_change
							}
							setSendingDisabled(isPartial)
						...
						case "command":
							if (!isAutoApproved(lastMessage) && !isPartial) {
								playSound("notification")
								showSystemNotification(t("hoodycode:notifications.command")) // hoodycode_change
							}
							setSendingDisabled(isPartial)
							setClineAsk("command")
							setEnableButtons(!isPartial)
						...
						case "condense":
							...
							setEnableButtons(!isPartial)
							setPrimaryButtonText(t("hoodycode:chat.condense.condenseConversation"))
							setSecondaryButtonText(undefined)
							break
						// hoodycode_change end
					}
	...
	const handlePrimaryButtonClick = useCallback(
		...
		[clineAsk, startNewTask, lastMessage?.text], // hoodycode_change: add lastMessage?.text
	)
	...
	const handleTaskCloseButtonClick = useCallback(() => startNewTask(), [startNewTask]) // hoodycode_change
	...
	// Request fork counts whenever the active task or task history changes
	useEffect(() => {
		if (currentTaskItem?.id) {
			vscode.postMessage({ type: "forkCountsRequest" })
		}
	}, [currentTaskItem?.id, taskHistoryVersion])
	...
	const isAllowedCommand = useCallback(
		(message: ClineMessage | undefined): boolean => {
			// hoodycode_change start wrap in try/catch
	...
	const groupedMessages = useMemo(() => {
		...
		visibleMessages.forEach((message: ClineMessage) => {
			// hoodycode_change start: upstream pr https://github.com/RooCodeInc/Roo-Code/pull/5452
	...
	const itemContent = useCallback(
		(index: number, messageOrGroup: ClineMessage | ClineMessage[]) => {
			...
			return (
				<ChatRow
					...
					highlighted={highlightedMessageIndex === index} // hoodycode_change: add highlight prop
					enableCheckpoints={enableCheckpoints} // hoodycode_change
		...
		[
			...
			highlightedMessageIndex, // hoodycode_change: add highlightedMessageIndex
			enableCheckpoints, // hoodycode_change
	...
	useEffect(() => {
		...
		window.addEventListener("wheel", handleWheel, { passive: true }) // hoodycode_change
		return () => {
			window.removeEventListener("keydown", handleKeyDown)
			window.removeEventListener("wheel", handleWheel) // hoodycode_change
		}
	}, [handleKeyDown, handleWheel]) // hoodycode_change
	...
	const showTelemetryBanner = telemetrySetting === "unset" // hoodycode_change
		...
		<div
			...
			{task ? (
				...
				<div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-4 relative">
					...
					{/* hoodycode_change start: changed the classes to support notifications */}
					<div className="w-full h-full flex flex-col gap-4 px-3.5 transition-all duration-300">
						{/* hoodycode_change end */}
						{/* Version indicator in top-right corner - only on welcome screen */}
						{/* hoodycode_change: do not show */}
						{/* <VersionIndicator
						...
						<div className="flex flex-grow flex-col justify-center gap-4">
							...
							{taskHistoryFullLength === 0 && <IdeaSuggestionsBox />} {/* hoodycode_change */}
							{/*<div className="mb-2.5">
								{cloudIsAuthenticated || taskHistory.length < 4 ? <RooTips /> : <RooCloudCTA />}
							</div> hoodycode_change: do not show */}
			...
			*/}
			{/* hoodycode_change: added settings toggle for this */}
				...
				<>
					<div className="grow flex" ref={scrollContainerRef}>
						<Virtuoso
							...
							increaseViewportBy={{ top: 400, bottom: 400 }} // hoodycode_change: use more modest numbers to see if they reduce gray screen incidence
			...
			/>
			{/* hoodycode_change: added settings toggle the profile and model selection */}
			<BottomControls showApiConfig />
			{/* hoodycode_change: end */}

			{/* hoodycode_change: disable {isProfileDisabled && (
				<div className="px-3">
