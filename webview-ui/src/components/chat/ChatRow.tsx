...
// import Thumbnails from "../common/Thumbnails" // hoodycode_change
import ImageBlock from "../common/ImageBlock"
import ErrorRow from "./ErrorRow"

import McpResourceRow from "../mcp/McpResourceRow"

// import { Mention } from "./Mention" // hoodycode_change
import { CheckpointSaved } from "./checkpoints/CheckpointSaved"
import { FollowUpSuggest } from "./FollowUpSuggest"
import { LowCreditWarning } from "../hoodycode/chat/LowCreditWarning" // hoodycode_change
...
import { NewTaskPreview } from "../hoodycode/chat/NewTaskPreview" // hoodycode_change
import { HoodyChatRowGutterBar } from "../hoodycode/chat/HoodyChatRowGutterBar" // hoodycode_change
import { AutoApprovedRequestLimitWarning } from "./AutoApprovedRequestLimitWarning"
import { CondenseContextErrorRow, CondensingContextRow, ContextCondenseRow } from "./ContextCondenseRow"
import CodebaseSearchResultsDisplay from "./CodebaseSearchResultsDisplay"
import { HoodyChatRowUserFeedback } from "../hoodycode/chat/HoodyChatRowUserFeedback" // hoodycode_change
import { StandardTooltip } from "../ui" // hoodycode_change
import { FastApplyChatDisplay } from "./hoodycode/FastApplyChatDisplay" // hoodycode_change
import { McpExecution } from "./McpExecution"
import { InvalidModelWarning } from "../hoodycode/chat/InvalidModelWarning"
import {
	ChevronRight,
	ChevronDown,
	Eye,
...
import { SeeNewChangesButtons } from "./hoodycode/SeeNewChangesButtons"
import ChatTimestamps from "./ChatTimestamps" // hoodycode_change
...
interface ChatRowProps {
	...
	highlighted?: boolean // hoodycode_change: Add highlighted prop
	onChatReset?: () => void // hoodycode_change
	enableCheckpoints?: boolean // hoodycode_change
	onFollowUpUnmount?: () => void
	isFollowUpAnswered?: boolean
	editable?: boolean
	// Fork indicator mapping: message timestamp -> number of forks
	forkCountsByTs?: Record<number, number>
}
...
const ChatRow = memo(
	(props: ChatRowProps) => {
		const { highlighted } = props // hoodycode_change: Add highlighted prop
		const { showTaskTimeline } = useExtensionState() // hoodycode_change: Used by HoodyChatRowGutterBar
		const { isLast, onHeightChange, message } = props
		// Store the previous height to compare with the current height
		// This allows us to detect changes without causing re-renders
		const prevHeightRef = useRef(0)
		const { t } = useTranslation()
		const forkCount = props.forkCountsByTs?.[message.ts] ?? 0

		const [chatrow, { height }] = useSize(
			<div
				// hoodycode_change: add highlighted className and group for hover actions
				className={cn(
					`px-[15px] py-[10px] pr-[6px] relative group ${highlighted ? "animate-message-highlight" : ""}`,
				)}>
				{showTaskTimeline && <HoodyChatRowGutterBar message={message} />}
				<ChatRowContent {...props} />

				{/* Fork from here action (visible on hover) */}
				<div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity">
					<StandardTooltip content={t("chat:fork.forkFromHere")}>
						<button
							aria-label={t("chat:fork.forkFromHere")}
							className="p-1 rounded hover:bg-vscode-list-hoverBackground text-vscode-descriptionForeground hover:text-vscode-foreground"
							onClick={(e) => {
								e.stopPropagation()
								if (typeof vscode !== "undefined" && vscode?.postMessage) {
									vscode.postMessage({ type: "forkTaskFromMessage", messageTs: message.ts })
								}
							}}
						>
				...
				{/* Fork counts indicator (if any forks branched from this message) */}
				{forkCount > 0 && (
					<div className="absolute bottom-1 right-1">
						<StandardTooltip content={t("chat:fork.branchedHere", { count: forkCount })}>
							<div className="flex items-center gap-1 px-1.5 py-0.5 rounded border text-[10px] leading-none bg-vscode-badge-background text-vscode-badge-foreground border-vscode-editorGroup-border">
								<span className="codicon codicon-git-branch scale-90" />
								<span>{t("chat:fork.branchedHere", { count: forkCount })}</span>
							</div>
						</StandardTooltip>
					</div>
				)}
...
export const ChatRowContent = ({
	...
	// hoodycode_change: add showTimestamps
	...
	// hoodycode_change: usageMissing, inferenceProvider
	...
	// hoodycode_change start: hide cost display check
	...
	// hoodycode_change end: hide cost display check
	...
	const [icon, title] = useMemo(() => {
			...
			case "api_req_started":
				...
				return [
					...
					apiReqCancelReason !== null && apiReqCancelReason !== undefined ? (
						...
						// hoodycode_change start: tooltip
		...
		t,
		isExpanded,
		inferenceProvider, // hoodycode_change
	])
	...
	switch (message.type) {
		case "say":
			switch (message.say) {
						...
						<>
							<div
								...
								</div>
								{
									// hoodycode_change start
									!cost && usageMissing && (
										<StandardTooltip content={t("hoodycode:pricing.costUnknownDescription")}>
											<div className="flex items-center text-xs text-vscode-dropdown-foreground border-vscode-dropdown-border/50 border px-1.5 py-0.5 rounded-lg whitespace-nowrap">
												...
												{t("hoodycode:pricing.costUnknown")}
											</div>
										</StandardTooltip>
									)
									// hoodycode_change end
				...
				case "completion_result":
					const commitRange = message.metadata?.hoodyCode?.commitRange
					return (
						<>
							<div style={headerStyle}>
								{icon}
					...
					return (
						<>
							<div style={headerStyle}>
								...
								<span style={{ color: normalColor, fontWeight: "bold" }}>
									{t("hoodycode:chat.condense.wantsToCondense")}
								</span>
							</div>
					...
					return (
						<>
							<div style={headerStyle}>
								...
								<span style={{ color: normalColor, fontWeight: "bold" }}>
									HoodyCode wants to create a Github issue:
								</span>
							</div>
