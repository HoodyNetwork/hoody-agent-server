...
import { IconProps } from "@radix-ui/react-icons/dist/types" // hoodycode_change
...
export interface DropdownOption {
	value: string
	label: string
	codicon?: string // hoodycode_change
	description?: string // hoodycode_change
	disabled?: boolean
...
export interface SelectDropdownProps {
	...
	disabled?: boolean
	initiallyOpen?: boolean // hoodycode_change
	title?: string
	...
	triggerIcon?: React.ForwardRefExoticComponent<IconProps & React.RefAttributes<SVGSVGElement>> | boolean | undefined // hoodycode_change
}
		...
		(
			{
				...
				onChange,
				disabled = false,
				initiallyOpen = false, // hoodycode_change
				title = "",
				triggerClassName = "",
				contentClassName = "",
				itemClassName = "",
				...
				renderItem,
				disableSearch = false,
				triggerIcon = CaretUpIcon, // hoodycode_change
			},
			ref,
		) => {
			const { t } = useTranslation()
			const [open, setOpen] = React.useState(initiallyOpen) // hoodycode_change
				...
				<Popover open={open} onOpenChange={onOpenChange} data-testid="dropdown-root">
					...
					<PopoverContent
						...
						<div className="flex flex-col w-full">
							...
							{/* hoodycode_change: different max height: max-h-82 */}
										...
										{groupedOptions.map((option, index) => {
											...
											return (
												<div
													...
													className={cn(
														"text-sm cursor-pointer flex items-center", // hoodycode_change
														option.disabled
															? "opacity-50 cursor-not-allowed"
