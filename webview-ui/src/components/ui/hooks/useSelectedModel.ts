import {
	...
	syntheticModels, // hoodycode_change
...
import { useExtensionState } from "@/context/ExtensionStateContext" // hoodycode_change

// hoodycode_change start
export const useModelProviders = (hoodycodeDefaultModel: string, apiConfiguration?: ProviderSettings) => {
	const provider = apiConfiguration?.apiProvider
	return useOpenRouterModelProviders(
		provider === "hoodycode"
			? (apiConfiguration?.hoodycodeModel ?? hoodycodeDefaultModel)
			: provider === "openrouter"
		...
		apiConfiguration?.hoodycodeOrganizationId ?? "personal",
	)
}
// hoodycode_change end
...
export const useSelectedModel = (apiConfiguration?: ProviderSettings) => {
	...
	// hoodycode_change start
	const { hoodycodeDefaultModel } = useExtensionState()
	const lmStudioModelId = provider === "lmstudio" ? apiConfiguration?.lmStudioModelId : undefined
	const ollamaModelId = provider === "ollama" ? apiConfiguration?.ollamaModelId : undefined

	const routerModels = useRouterModels({
		openRouterBaseUrl: apiConfiguration?.openRouterBaseUrl,
		openRouterApiKey: apiConfiguration?.apiKey, // hoodycode_change
		hoodycodeOrganizationId: apiConfiguration?.hoodycodeOrganizationId, // hoodycode_change
	})
	const openRouterModelProviders = useModelProviders(hoodycodeDefaultModel, apiConfiguration)
	// hoodycode_change end
...
function getSelectedModel({
	...
	switch (provider) {
		...
		case "chutes": {
			...
			const info = routerModels.chutes[id] // hoodycode_change
			return { id, info }
		}
		case "bedrock": {
		...
		case "hoodycode": {
			...
			if (routerModels["hoodycode-openrouter"] && apiConfiguration.hoodycodeModel) {
				// Find the model in the fetched models
				const modelEntries = Object.entries(routerModels["hoodycode-openrouter"])

				const selectedModelId = apiConfiguration.hoodycodeModel.toLowerCase()
			...
			const invalidOrDefaultModel = apiConfiguration.hoodycodeModel ?? hoodycodeDefaultModel
			return {
				id: invalidOrDefaultModel,
				info: routerModels["hoodycode-openrouter"][invalidOrDefaultModel],
			}
		}
		case "gemini-cli": {
