...
import ai.hoodycode.jetbrains.commands.CommandRegistry
import ai.hoodycode.jetbrains.commands.ICommand
import ai.hoodycode.jetbrains.util.URI
import ai.hoodycode.jetbrains.util.URIComponents
import java.io.File
    ...
    suspend fun vscode_changes(title: String, changes: List<List<Map<String, Any?>>>): Any? {
        ...
        try {
            ...
            for ((index, changeTriplet) in changes.withIndex()) {
                ...
                val originalFileUri = ai.hoodycode.jetbrains.editor.createURI(changeTriplet[0])
                val beforeContentUri = ai.hoodycode.jetbrains.editor.createURI(changeTriplet[1])
                val afterContentUri = ai.hoodycode.jetbrains.editor.createURI(changeTriplet[2])
