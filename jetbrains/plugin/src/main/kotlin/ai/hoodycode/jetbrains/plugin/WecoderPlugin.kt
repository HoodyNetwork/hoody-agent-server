...
package ai.hoodycode.jetbrains.plugin
...
import ai.hoodycode.jetbrains.core.ExtensionProcessManager
import ai.hoodycode.jetbrains.core.ExtensionSocketServer
import ai.hoodycode.jetbrains.core.ServiceProxyRegistry
import ai.hoodycode.jetbrains.webview.WebViewManager
import ai.hoodycode.jetbrains.workspace.WorkspaceFileChangeManager
...
import ai.hoodycode.jetbrains.core.*
import ai.hoodycode.jetbrains.util.ExtensionUtils
import ai.hoodycode.jetbrains.util.PluginConstants
import ai.hoodycode.jetbrains.util.PluginResourceUtil
import java.io.File
...
class WecoderPlugin : StartupActivity.DumbAware {
    ...
    override fun runActivity(project: Project) {
        ...
        LOG.info(
            "Initializing Hoody Code plugin for project: ${project.name}, " +
        ...
        try {
            ...
            Disposer.register(project, Disposable {
                LOG.info("Disposing Hoody Code plugin for project: ${project.name}")
                pluginService.dispose()
                // SystemObjectProvider is now project-scoped and will be disposed automatically
            })

            LOG.info("Hoody Code plugin initialized successfully for project: ${project.name}")
        } catch (e: Exception) {
            LOG.error("Failed to initialize Hoody Code plugin", e)
        }
    }
}
...
class WecoderPluginService(private var currentProject: Project) : Disposable {
    ...
    companion object {
        // Debug mode switch
        @Volatile
        private var DEBUG_TYPE: DEBUG_MODE = ai.hoodycode.jetbrains.plugin.DEBUG_MODE.NONE
        ...
        init {
            try {
                ...
                val configStream: InputStream? = WecoderPluginService::class.java.getResourceAsStream("/ai/hoodycode/jetbrains/plugin/config/plugin.properties")
...
//                ServiceProxyRegistry.getInstance().initialize()
                ...
                if (DEBUG_TYPE == ai.hoodycode.jetbrains.plugin.DEBUG_MODE.ALL) {
    ...
    private fun cleanup() {
        ...
        try {
            if (DEBUG_TYPE == ai.hoodycode.jetbrains.plugin.DEBUG_MODE.NONE) {
