...
package ai.hoodycode.jetbrains.commands
...
import ai.hoodycode.jetbrains.core.PluginContext
...
/**
 * JetBrains Protocol Command for handling Hoody Code authentication URLs
 *
 * Handles URLs in the format: jetbrains://idea/ai.hoodycode.jetbrains.auth?token=HERE
 * and forwards them to the VSCode extension via RPC protocol
 */
class HoodyCodeAuthProtocolCommand : JBProtocolCommand("ai.hoodycode.jetbrains.auth") {
    private val logger = Logger.getInstance(HoodyCodeAuthProtocolCommand::class.java)
    private val coroutineScope = CoroutineScope(SupervisorJob() + Dispatchers.IO)
    
    companion object {
        const val COMMAND_ID = "ai.hoodycode.jetbrains.auth"
    ...
    override suspend fun execute(target: String?, parameters: Map<String, String>, fragment: String?): String? {
        logger.info("Handling Hoody Code auth protocol command: target=$target, parameters=$parameters")
        ...
        return try {
            ...
            val errorMsg = "Error handling Hoody Code auth protocol command: ${e.message}"
    ...
    private fun forwardTokenToVSCodeExtension(token: String) {
        coroutineScope.launch {
            try {
                ...
                val vscodeUriString = "vscode://hoodycode.hoody-code/hoodycode?token=${token}"
