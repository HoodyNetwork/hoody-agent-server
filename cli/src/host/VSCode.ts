...
export class ExtensionContext {
	...
	constructor(extensionPath: string, workspacePath: string) {
		...
		HoodyCodePaths.initializeWorkspace(workspacePath)

		const globalStoragePath = HoodyCodePaths.getGlobalStorageDir()
		const workspaceStoragePath = HoodyCodePaths.getWorkspaceStorageDir(workspacePath)
		const logPath = HoodyCodePaths.getLogsDir()
...
export class MockWorkspaceConfiguration implements WorkspaceConfiguration {
	...
	constructor(section?: string, context?: ExtensionContext) {
		...
		if (context) {
			...
			const globalStoragePath = HoodyCodePaths.getGlobalStorageDir()
			const workspaceStoragePath = HoodyCodePaths.getWorkspaceStorageDir(process.cwd())
	...
	return {
		...
		extensions: {
			...
			getExtension: (extensionId: string) => {
				...
				if (extensionId === "hoodycode.hoody-code") {
					return {
						id: extensionId,
