...
export class ExtensionHost extends EventEmitter {
	...
	private async setupVSCodeAPIMock(): Promise<void> {
		...
		process.env.HOODY_CLI_MODE = "true"
		...
		if (this.vscodeAPI && this.vscodeAPI.context) {
			...
			this.on("webviewMessage", async (message: any) => {
				await this.safeExecute(async () => {
					...
					const webviewProvider = this.webviewProviders.get("hoody-code.SidebarProvider")
	...
	private initializeState(): void {
		...
		this.currentState = {
			...
			renderContext: "cli",
			telemetrySetting: "disabled", // Privacy-first: disabled by default
