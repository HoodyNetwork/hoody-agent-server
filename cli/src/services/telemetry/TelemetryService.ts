...
import { HOODYCODE_POSTHOG_API_KEY } from "../../constants/telemetry.js"
...
export class TelemetryService {
	...
	public async initialize(
		...
		try {
			...
			const apiKey = HOODYCODE_POSTHOG_API_KEY
			if (!apiKey) {
				logs.warn("HOODYCODE_POSTHOG_API_KEY not set, telemetry disabled", "TelemetryService")
				this.isInitialized = true
				return
			}
			...
			// Update Hoodycode user ID if token is available
			const provider = config.providers.find((p) => p.id === config.provider)
			if (provider && provider.hoodycodeToken) {
				await identityManager.updateHoodycodeUserId(provider.hoodycodeToken)
			}
			...
			const telemetryConfig: TelemetryConfig = {
				...
				debug: process.env.HOODY_TELEMETRY_DEBUG === "true",
			}
	...
	/**
	 * Update Hoodycode user ID
	 */
	public async updateHoodycodeUserId(hoodycodeToken: string): Promise<void> {
		if (this.client) {
			await this.client.updateHoodycodeUserId(hoodycodeToken)
		}
	}

	/**
	 * Clear Hoodycode user ID
	 */
	public clearHoodycodeUserId(): void {
		if (this.client) {
			this.client.clearHoodycodeUserId()
		}
	}
