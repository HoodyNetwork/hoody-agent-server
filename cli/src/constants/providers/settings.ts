...
export const FIELD_REGISTRY: Record<string, FieldMetadata> = {
	// Hoodycode fields
	hoodycodeToken: {
		label: "Hoody Code Token",
		type: "password",
		placeholder: "Enter your Hoody Code token...",
	},
...
export const isOptionalField = (field: string): boolean => {
	...
	// Fallback logic for fields not in registry
	return field.includes("BaseUrl") || field === "hoodycodeOrganizationId"
}
...
export const getProviderSettings = (provider: ProviderName, config: ProviderSettings): ProviderSettingConfig[] => {
	switch (provider) {
		case "hoodycode":
			return [
				createFieldConfig("hoodycodeToken", config),
				createFieldConfig("hoodycodeOrganizationId", config, "personal"),
				createFieldConfig("hoodycodeModel", config, "anthropic/claude-sonnet-4"),
			]
...
export const PROVIDER_DEFAULT_MODELS: Record<ProviderName, string> = {
	hoodycode: "anthropic/claude-sonnet-4",
