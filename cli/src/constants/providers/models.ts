...
export const PROVIDER_TO_ROUTER_NAME: Record<ProviderName, RouterName | null> = {
	hoodycode: "hoodycode-openrouter",
 ...
 * Mirrors the logic from webview-ui/src/components/hoodycode/hooks/useProviderModels.ts
 *
...
export function getModelsByProvider(params: {
	...
	const { provider, routerModels, hoodycodeDefaultModel } = params
	...
	if (routerName && routerModels && routerModels[routerName]) {
		...
		return {
			...
			defaultModel: provider === "hoodycode" ? hoodycodeDefaultModel : defaultModelId,
		}
	}
 ...
 * Mirrors the logic from webview-ui/src/components/hoodycode/hooks/useSelectedModel.ts
 */
...
export function getCurrentModelId(params: {
	...
	const { providerConfig, routerModels, hoodycodeDefaultModel } = params
 ...
 * Mirrors the logic from webview-ui/src/components/ui/hooks/hoodycode/usePreferredModels.ts
 */
