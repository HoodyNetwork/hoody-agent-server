	...
	const plugins = [
		{
			...
			setup(build) {
				build.onEnd(() => {
					...
					// Copy OpenAPI spec to project root
					const openapiSource = path.join(srcDir, "services/api-server/generated/swagger.yaml")
					const openapiDest = path.join(srcDir, "../OPENAPI.yaml")
					
					if (fs.existsSync(openapiSource)) {
						fs.copyFileSync(openapiSource, openapiDest)
						console.log(`[${name}] Copied OpenAPI spec to root: OPENAPI.yaml`)
					} else {
						console.warn(`[${name}] OpenAPI spec not found at: ${openapiSource}`)
						console.warn(`[${name}] Run 'pnpm tsoa' to generate the spec first`)
					}
