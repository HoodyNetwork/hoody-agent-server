...
describe("DEFAULT_HEADERS", () => {
	...
	it("should have correct HTTP-Referer value", () => {
		expect(DEFAULT_HEADERS["HTTP-Referer"]).toBe("https://hoody.com")
	})

	it("should have correct X-Title value", () => {
		expect(DEFAULT_HEADERS["X-Title"]).toBe("Hoody Code")
	})

	it("should have correct User-Agent format", () => {
		const userAgent = DEFAULT_HEADERS["User-Agent"]
		expect(userAgent).toBe(`Hoody-Code/${Package.version}`)
	...
	it("should have User-Agent with correct tool name", () => {
		const userAgent = DEFAULT_HEADERS["User-Agent"]
		expect(userAgent.startsWith("Hoody-Code/")).toBe(true)
	})
	...
	it("should have exactly 4 headers", () => {
		...
		expect(headerKeys).toEqual(["HTTP-Referer", "X-Title", "X-HoodyCode-Version", "User-Agent"])
	})
})
