...
export class NativeOllamaHandler extends BaseProvider implements SingleCompletionHandler {
	...
	private isInitialized = false // hoodycode_change

	constructor(options: ApiHandlerOptions) {
		super()
		this.options = options
		this.initialize() // hoodycode_change
	}
	...
	override async *createMessage(
		...
		const { id: modelId, info: modelInfo } = this.getModel() // hoodycode_change: fetchModel => getModel
	...
	override getModel(): { id: string; info: ModelInfo } {
		...
		return {
			id: modelId,
			info: modelInfo, // hoodycode_change
		}
	}
	...
	async completePrompt(prompt: string): Promise<string> {
		try {
			...
			const client = this.ensureClient()
			const { id: modelId } = this.getModel() // hoodycode_change: fetchModel => getModel
			const useR1Format = modelId.toLowerCase().includes("deepseek-r1")
