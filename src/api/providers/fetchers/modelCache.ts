...
export /*hoodycode_change*/ async function writeModels(router: RouterName, data: ModelRecord) {
	const filename = `${router}_models.json`
	const cacheDir = await getCacheDirectoryPath(ContextProxy.instance.globalStorageUri.fsPath)
	await safeWriteJson(path.join(cacheDir, filename), data)
}

export /*hoodycode_change*/ async function readModels(router: RouterName): Promise<ModelRecord | undefined> {
	const filename = `${router}_models.json`
	const cacheDir = await getCacheDirectoryPath(ContextProxy.instance.globalStorageUri.fsPath)
	...
	try {
		switch (provider) {
			...
			case "hoodycode-openrouter":
				models = await getOpenRouterModels({
					openRouterBaseUrl: "https://ai.hoody.run/api/v1",
					headers: options.hoodycodeToken ? { Authorization: `Bearer ${options.hoodycodeToken}` } : undefined,
				})
				break
			case "chutes":
			...
			// hoodycode_change end
			case "ollama":
				models = await getOllamaModels(options.baseUrl, options.apiKey, options.numCtx /*hoodycode_change*/)
				break
			case "lmstudio":
		...
		/* hoodycode_change: skip useless file IO
