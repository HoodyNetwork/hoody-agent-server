...
import { DEFAULT_HEADERS } from "../constants" // hoodycode_change
...
const modelRouterBaseModelSchema = z.object({
	...
	preferredIndex: z.number().nullish(), // hoodycode_change
...
export const openRouterModelEndpointSchema = modelRouterBaseModelSchema.extend({
	model_name: z.string(), // hoodycode_change
...
export async function getOpenRouterModels(
	options?: ApiHandlerOptions & { headers?: Record<string, string> }, // hoodycode_change: added headers
): Promise<Record<string, ModelInfo>> {
	const models: Record<string, ModelInfo> = {}
	const baseURL = options?.openRouterBaseUrl || "https://openrouter.ai/api/v1"

	try {
		// hoodycode_change: use fetch, added headers
		...
		for (const model of data) {
			...
			models[id] = parseOpenRouterModel({
				id,
				model,
				displayName: model.name, // hoodycode_change
	...
	try {
		...
		for (const endpoint of endpoints) {
			models[endpoint.tag ?? endpoint.provider_name] = parseOpenRouterModel({
				id,
				model: endpoint,
				displayName: endpoint.model_name, // hoodycode_change
				inputModality: architecture?.input_modalities,
				outputModality: architecture?.output_modalities,
...
export const parseOpenRouterModel = ({
	...
	displayName?: string // hoodycode_change
