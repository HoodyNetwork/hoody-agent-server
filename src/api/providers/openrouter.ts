...
import type {
	ApiHandlerCreateMessageMetadata, // hoodycode_change
	SingleCompletionHandler,
} from "../index"
import { verifyFinishReason } from "./hoodycode/verifyFinishReason"
...
import { isAnyRecognizedHoodyCodeError } from "../../shared/hoodycode/errorUtils"
...
interface CompletionUsage {
	...
	total_tokens?: number
	cost?: number
	is_byok?: boolean // hoodycode_change
	cost_details?: {
...
export class OpenRouterHandler extends BaseProvider implements SingleCompletionHandler {
	...
	// hoodycode_change start property
	protected get providerName(): "OpenRouter" | "HoodyCode" {
	...
	override async *createMessage(
		...
		metadata?: ApiHandlerCreateMessageMetadata, // hoodycode_change
		...
		const completionParams: OpenRouterChatCompletionParams = {
			...
			...this.getProviderParams(), // hoodycode_change: original expression was moved into function
		...
		try {
			stream = await this.client.chat.completions.create(
				completionParams,
				this.customRequestOptions(metadata), // hoodycode_change
			)
		} catch (error) {
			// hoodycode_change start
			if (this.providerName == "HoodyCode" && isAnyRecognizedHoodyCodeError(error)) {
				throw error
			}
		...
		let inferenceProvider: string | undefined // hoodycode_change
		...
		try {
			for await (const chunk of stream) {
				...
				verifyFinishReason(chunk.choices[0]) // hoodycode_change
				const delta = chunk.choices[0]?.delta

				if (
					delta /* hoodycode_change */ &&
					"reasoning" in delta &&
					delta.reasoning &&
	...
	async completePrompt(prompt: string) {
		...
		try {
			response = await this.client.chat.completions.create(
				completionParams,
				this.customRequestOptions(), // hoodycode_change
			)
		} catch (error) {
			throw handleOpenAIError(error, this.providerName)
		}
	...
	async generateImage(
		...
		apiKey: string,
		inputImage?: string,
		taskId?: string, // hoodycode_change
		...
		try {
			const response = await fetch(
				`${this.options.openRouterBaseUrl || "https://openrouter.ai/api/v1/"}chat/completions`, // hoodycode_change: support baseUrl
				{
					method: "POST",
					headers: {
						// hoodycode_change start
