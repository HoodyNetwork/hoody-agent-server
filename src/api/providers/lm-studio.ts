...
import { fetchWithTimeout, HeadersTimeoutError } from "./hoodycode/fetchWithTimeout"

import { getModels, getModelsFromCache } from "./fetchers/modelCache"
import { handleOpenAIError } from "./utils/openai-error-handler"
import { getApiRequestTimeout } from "./utils/timeout-config" // hoodycode_change
...
export class LmStudioHandler extends BaseProvider implements SingleCompletionHandler {
	...
	constructor(options: ApiHandlerOptions) {
		...
		const timeout = getApiRequestTimeout() // hoodycode_change
	...
	override async *createMessage(
		...
		try {
			...
			// hoodycode_change end
			throw new Error(
				"Please check the LM Studio developer logs to debug what went wrong. You may need to load the model with a larger context length to work with Hoody Code's prompts.",
			)
		}
	}
