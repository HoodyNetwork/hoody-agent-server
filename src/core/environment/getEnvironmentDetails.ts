...
export async function getEnvironmentDetails(cline: Task, includeFileDetails: boolean = false) {
	...
	if (cline.api instanceof OpenRouterHandler || cline.api instanceof NativeOllamaHandler) {
		try {
			...
			await cline.say(
				"error",
				t("hoodycode:task.notLoggedInError", { error: e instanceof Error ? e.message : String(e) }),
			)
	...
	const currentMode = modeDetails.slug ?? mode // hoodycode_change: don't try to use non-existent modes
