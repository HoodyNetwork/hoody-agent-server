...
import axios from "axios" // hoodycode_change

import { type ModeConfig, type PromptComponent, customModesSettingsSchema, modeConfigSchema } from "@roo-code/types"

import { fileExistsAtPath } from "../../utils/fs"
import { getWorkspacePath } from "../../utils/path"
import { getGlobalRooDirectory, getProjectRooDirectoryForCwd /*hoodycode_change*/ } from "../../services/roo-config"
...
import { X_HOODYCODE_ORGANIZATIONID, X_HOODYCODE_TESTER } from "../../shared/hoodycode/headers"
// hoodycode_change end

const ROOMODES_FILENAME = ".hoodycodemodes"
...
export class CustomModesManager {
	...
	// hoodycode_change start: Added organizationModes parameter and precedence logic
	...
	private async watchCustomModesFiles(): Promise<void> {
		...
		const handleSettingsChange = async () => {
			try {
				...
				// Get modes from .hoodycodemodes if it exists (takes precedence)
				const roomodesPath = await this.getWorkspaceRoomodes()
				const roomodesModes = roomodesPath ? await this.loadModesFromFile(roomodesPath) : []

				// hoodycode_change start Get organization modes from global state to preserve them
		...
		if (workspaceFolders && workspaceFolders.length > 0) {
			...
			const handleRoomodesChange = async () => {
				try {
					...
					console.error(`[CustomModesManager] Error handling .hoodycodemodes file change:`, error)
				}
			}
	...
	public async getCustomModes(): Promise<ModeConfig[]> {
		...
		// Get modes from .hoodycodemodes if it exists
		const roomodesPath = await this.getWorkspaceRoomodes()
		const roomodesModes = roomodesPath ? await this.loadModesFromFile(roomodesPath) : []

		// hoodycode_change start: Get organization modes from global state
		...
		try {
			...
			if (isGlobalMode) {
				...
				modeRulesDir = path.join(
					getProjectRooDirectoryForCwd(workspacePath) /* hoodycode_change */,
					`rules-${slug}`,
				)
			}
	...
	public async exportModeWithRules(slug: string, customPrompts?: PromptComponent): Promise<ExportResult> {
		try {
			...
			const modeRulesDir = isGlobalMode
				...
				: path.join(getProjectRooDirectoryForCwd(baseDir) /* hoodycode_change */, `rules-${slug}`)
	...
	private async importRulesFiles(
		...
		if (source === "global") {
			...
			const workspacePath = getWorkspacePath()
			baseDir = getProjectRooDirectoryForCwd(workspacePath) // hoodycode_change
			rulesFolderPath = path.join(baseDir, `rules-${importMode.slug}`)
		}
	...
	// hoodycode_change start: New method to fetch organization modes
	/**
	 * Fetches custom modes for a specific organization from the HoodyCode API
	 * @param hoodycodeToken - The authentication token
	 * @param organizationId - The organization ID (undefined for personal account)
	 * @param hoodycodeTesterWarningsDisabledUntil - Timestamp for suppressing tester warnings
	 * @returns Array of organization-specific modes
	 */
	public async fetchOrganizationModes(
		hoodycodeToken: string,
		organizationId?: string,
		hoodycodeTesterWarningsDisabledUntil?: number,
	): Promise<ModeConfig[]> {
		try {
			...
			headers[X_HOODYCODE_ORGANIZATIONID] = organizationId

			// Add X-HOODYCODE-TESTER: SUPPRESS header if the setting is enabled
			if (hoodycodeTesterWarningsDisabledUntil && hoodycodeTesterWarningsDisabledUntil > Date.now()) {
				headers[X_HOODYCODE_TESTER] = "SUPPRESS"
			}
	...
	// hoodycode_change start: New method to refresh modes with organization modes
	/**
