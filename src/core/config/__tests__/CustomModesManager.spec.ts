...
import { getProjectRooDirectoryForCwd } from "../../../services/roo-config" // hoodycode_change
...
describe("CustomModesManager", () => {
	...
	const mockRoomodes = path.join(mockWorkspacePath, ".hoodycodemodes")
	...
	describe("getCustomModes", () => {
		...
		it("should merge modes with .hoodycodemodes taking precedence", async () => {
			const settingsModes = [
			...
			// mode2 should come from .hoodycodemodes since it takes precedence
		...
		it("should handle missing .hoodycodemodes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
		...
		it("should handle invalid YAML in .hoodycodemodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .hoodycodemodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .hoodycodemodes precedence", async () => {
			...
			// Should update global state with merged modes where .hoodycodemodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Roomodes Mode 1", // .hoodycodemodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .hoodycodemodes file when adding project-specific mode", async () => {
			...
			// Mock .hoodycodemodes to not exist initially
			...
			// Verify .hoodycodemodes was created with the project mode
			...
			// Verify the content written to .hoodycodemodes
	...
	describe("updateModesInFile", () => {
		...
		describe("importModeWithRules", () => {
			...
			it("should successfully import mode without rules files", async () => {
				...
				expect(fs.writeFile).toHaveBeenCalledWith(
					expect.stringContaining(".hoodycodemodes"),
			...
			it("should prevent path traversal attacks in import", async () => {
				...
				const mockWorkspacePath = path.resolve("/mock/workspace")
				const writtenRuleFiles = writtenFiles.filter((p) => !p.includes(".hoodycodemodes"))
				writtenRuleFiles.forEach((filePath) => {
					const normalizedPath = path.normalize(filePath)
					const expectedBasePath = path.normalize(
						getProjectRooDirectoryForCwd(mockWorkspacePath), // hoodycode_change
					)
	...
	customModes:
			...
			it("should remove existing rules folder when importing mode without rules", async () => {
				...
				expect(fs.rm).toHaveBeenCalledWith(expect.stringContaining(path.join(".hoodycode", "rules-test-mode")), {
					recursive: true,
					force: true,
				})
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should work with global custom modes when .roomodes doesn't exist", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.hoodycode/rules-test-mode" },
			])
	...
	describe("exportModeWithRules", () => {
		...
		it("should successfully export mode with rules for a built-in mode customized in .roomodes", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.hoodycode/rules-code" },
			])
