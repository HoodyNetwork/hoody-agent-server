...
import { migrateMorphApiKey } from "./hoodycode/migrateMorphApiKey"
...
export const providerProfilesSchema = z.object({
	...
	migrations: z
		.object({
			...
			morphApiKeyMigrated: z.boolean().optional(), // hoodycode_change: Morph API key migration
		})
		.optional(),
})
			...
			return await this.lock(async () => {
				...
				if (!providerProfiles.migrations) {
					providerProfiles.migrations = {
						...
						morphApiKeyMigrated: false, // hoodycode_change: Morph API key migration
