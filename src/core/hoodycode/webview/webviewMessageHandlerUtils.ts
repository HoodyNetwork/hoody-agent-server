...
const resendMessageSequence = async (
	...
	if (!currentCline || currentCline.taskId !== taskId) {
		...
		vscode.window.showErrorMessage(t("hoodycode:userFeedback.message_update_failed"))
		return false
	}
...
export const fetchHoodycodeNotificationsHandler = async (provider: ClineProvider) => {
	try {
		const { apiConfiguration } = await provider.getState()
		const hoodycodeToken = apiConfiguration?.hoodycodeToken

		if (!hoodycodeToken || apiConfiguration?.apiProvider !== "hoodycode") {
		...
		const response = await axios.get(`${getHoodyBaseUriFromToken(hoodycodeToken)}/api/users/notifications`, {
			headers,
			timeout: 5000,
		})
		...
		provider.log(`Error fetching Hoodycode notifications: ${error.message}`)
		provider.postMessageToWebview({
			type: "hoodycodeNotificationsResponse",
			notifications: [],
		})
	...
	try {
		...
		if (revert) {
			...
			if (checkpointMessage && checkpointMessage.text) {
				...
				vscode.window.showErrorMessage(t("hoodycode:userFeedback.no_checkpoint_found"))
			}
		}
		...
		if (success) {
			vscode.window.showInformationMessage(t("hoodycode:userFeedback.message_updated"))
		}
	} catch (error) {
