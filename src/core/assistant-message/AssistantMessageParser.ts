...
export class AssistantMessageParser {
	...
	private accumulator = ""
	private rawAccumulator = "" // Store original before transformation
	 ...
	 * Variant 3: <function_calls><invoke name="tool"><parameter name="x">...</parameter></invoke></function_calls>
	 * Output: <tool><x>...</x></tool>
	 */
	private transformFunctionCallsFormat(text: string): string {
		// Pattern to match <function_calls>...</function_calls> blocks
		const functionCallsPattern = /<function_calls>([\s\S]*?)<\/function_calls>/g

		// Normalize parameter wrappers from Anthropic's Haiku 4.5 (<parameter name="...">)
		const normalizeParams = (content: string): string => {
			let result = content
			// Remove generic parameter containers if present
			result = result.replace(/<\/?parameters>/g, "")
			// Convert <parameter name="param">...</parameter> to <param>...</param>
		...
		return text.replace(functionCallsPattern, (match, innerContent) => {
			...
			let transformedContent = innerContent.replace(invokeNamePattern, (_: string, toolName: string, paramsContent: string) => {
				...
				// Normalize parameters (handles <parameter name="...">)
	...
	public processChunk(chunk: string): AssistantMessageContent[] {
		...
		const transformedChunk = this.transformFunctionCallsFormat(chunk)
		
		if (this.accumulator.length + transformedChunk.length > this.MAX_ACCUMULATOR_SIZE) {
