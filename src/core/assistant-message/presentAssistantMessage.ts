...
import { editFileTool } from "../tools/editFileTool" // hoodycode_change: Morph fast apply
...
import { newRuleTool } from "../tools/newRuleTool" // hoodycode_change
import { reportBugTool } from "../tools/reportBugTool" // hoodycode_change
import { condenseTool } from "../tools/condenseTool" // hoodycode_change
import { codebaseSearchTool } from "../tools/codebaseSearchTool"
import { experiments, EXPERIMENT_IDS } from "../../shared/experiments"
import { applyDiffToolLegacy } from "../tools/applyDiffTool"
import { yieldPromise } from "../hoodycode"
...
export async function presentAssistantMessage(cline: Task, recursionDepth: number = 0 /*hoodycode_change*/) {
	if (cline.abort) {
		...
		case "tool_use":
			const toolDescription = (): string => {
				switch (block.name) {
					...
					case "search_and_replace":
						return `[${block.name} for '${block.params.path}']`
					// hoodycode_change start: Morph fast apply
					case "edit_file":
			...
			const askApproval = async (
				...
				if (provider) {
					...
					// Master switch: autoApprovalEnabled must be true
					if (state?.autoApprovalEnabled) {
						// Check specific operation permissions based on ask type
						let shouldAutoApprove = false
						
						switch (type) {
							case "tool":
								// Parse tool message to determine operation type
								try {
									const toolData = partialMessage ? JSON.parse(partialMessage) : {}
									const toolName = toolData.tool
									const isOutsideWorkspace = toolData.isOutsideWorkspace === true
									const isProtectedFile = isProtected === true
									
									// Read-only operations
									if (["readFile", "listFilesTopLevel", "listFilesRecursive", "listCodeDefinitionNames", "searchFiles", "codebaseSearch"].includes(toolName)) {
										shouldAutoApprove = state.alwaysAllowReadOnly === true
										if (shouldAutoApprove && isOutsideWorkspace) {
											shouldAutoApprove = state.alwaysAllowReadOnlyOutsideWorkspace === true
										}
									}
									// Write operations
									else if (["editedExistingFile", "newFileCreated", "appliedDiff", "searchAndReplace", "insertContent"].includes(toolName)) {
										shouldAutoApprove = state.alwaysAllowWrite === true
										if (shouldAutoApprove && isOutsideWorkspace) {
											shouldAutoApprove = state.alwaysAllowWriteOutsideWorkspace === true
										}
										if (shouldAutoApprove && isProtectedFile) {
											shouldAutoApprove = state.alwaysAllowWriteProtected === true
										}
									}
									// Image generation (write operation)
									else if (toolName === "generateImage" || toolName === "imageGenerated") {
										shouldAutoApprove = state.alwaysAllowWrite === true
										if (shouldAutoApprove && isOutsideWorkspace) {
											shouldAutoApprove = state.alwaysAllowWriteOutsideWorkspace === true
										}
									...
									// Mode operations
										...
										shouldAutoApprove = state.alwaysAllowModeSwitch === true
									}
									// Subtask operations
									else if (toolName === "newTask" || toolName === "finishTask") {
										shouldAutoApprove = state.alwaysAllowSubtasks === true
									}
									// Todo list operations
									else if (toolName === "updateTodoList") {
										shouldAutoApprove = state.alwaysAllowUpdateTodoList === true
									}
								} catch (error) {
							...
							case "command":
								// Command execution
								shouldAutoApprove = state.alwaysAllowExecute === true
								break
								
							case "browser_action_launch":
								// Browser operations
								shouldAutoApprove = state.alwaysAllowBrowser === true
								break
								
							case "use_mcp_server":
								// MCP operations
								shouldAutoApprove = state.alwaysAllowMcp === true
								break
								
							case "followup":
								// Followup questions - handled differently (uses timeout in tool)
								// Don't auto-approve here, let the tool handle it
								shouldAutoApprove = false
								break
								
							default:
								// Unknown ask types are not auto-approved
								shouldAutoApprove = false
						}
						
						// If auto-approved, return immediately without asking
				...
				// Not auto-approved, ask the user
			...
			await checkpointSaveAndMark(cline) // hoodycode_change: moved out of switch
			switch (block.name) {
				case "write_to_file":
	...
	if (!block.partial || cline.didRejectTool || cline.didAlreadyUseTool) {
		...
		if (cline.currentStreamingContentIndex < cline.assistantMessageContent.length) {
			...
			// hoodycode_change start: prevent excessive recursion
			await yieldPromise()
