...
describe("RooIgnoreController", () => {
	...
	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .hoodycodeignore exists
		 */
		it("should load .hoodycodeignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .hoodycodeignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".hoodycodeignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".hoodycodeignore"), "utf8")
		...
		/**
		 * Tests the controller behavior when .hoodycodeignore doesn't exist
		 */
		it("should allow all access when .hoodycodeignore doesn't exist", async () => {
			// Setup mocks to simulate missing .hoodycodeignore file
		 ...
		 */
		it("should set up file watcher for .hoodycodeignore changes", async () => {
			// Check that watcher was created with correct pattern
			expect(vscode.workspace.createFileSystemWatcher).toHaveBeenCalledWith(
				expect.objectContaining({
					base: TEST_CWD,
					pattern: ".hoodycodeignore",
				}),
			)
		 ...
		 */
		it("should handle errors when loading .hoodycodeignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .hoodycodeignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .hoodycodeignore exists
		 */
		it("should allow all access when no .hoodycodeignore content", async () => {
			// Create a new controller with no .hoodycodeignore
			mockFileExists.mockResolvedValue(false)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .hoodycodeignore exists
		 */
		it("should allow all commands when no .hoodycodeignore exists", async () => {
			// Create a new controller with no .hoodycodeignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new RooIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .hoodycodeignore
		 */
		it("should generate formatted instructions when .hoodycodeignore exists", async () => {
			// Setup .hoodycodeignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
			...
			expect(instructions).toContain("# .hoodycodeignore")
		 ...
		 * Tests behavior when no .hoodycodeignore exists
		 */
		it("should return undefined when no .hoodycodeignore exists", async () => {
			// Setup no .hoodycodeignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .hoodycodeignore is created
		 */
		it("should reload .hoodycodeignore when file is created", async () => {
			// Setup initial state without .hoodycodeignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .hoodycodeignore
			controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .hoodycodeignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .hoodycodeignore is changed
		 */
		it("should reload .hoodycodeignore when file is changed", async () => {
			// Setup initial state with .hoodycodeignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .hoodycodeignore is deleted
		 */
		it("should reset when .hoodycodeignore is deleted", async () => {
			// Setup initial state with .hoodycodeignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
