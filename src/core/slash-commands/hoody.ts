 ...
 * and processes Hoody-specific slash commands. It should be merged with parseSlashCommands in the future.
 */
export async function parseHoodySlashCommands(
	text: string,
	...
	const commandReplacements: Record<string, ((userInput: string) => string) | undefined> = {
		...
		remember: rememberToolResponse,
		mb: rememberToolResponse, // Alias for remember
		revisetodo: reviseTodoToolResponse,
	}
	...
	for (const { regex } of tagPatterns) {
		...
		if (match) {
			...
			if (command) {
				...
				return {
					processedText,
					needsRulesFileCheck: commandName === "newrule" || commandName === "remember" || commandName === "mb"
				}
			}
