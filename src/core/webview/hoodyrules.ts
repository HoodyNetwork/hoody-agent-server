...
function getRuleDirectoryPath(baseDir: string, ruleType: "rule" | "workflow") {
	return ruleType === "workflow"
		...
		: path.join(baseDir, GlobalFileNames.hoodyRules)
}
...
export async function createRuleFile(
	...
	if (!workspacePath && !isGlobal) {
		vscode.window.showErrorMessage(t("hoodycode:rules.errors.noWorkspaceFound"))
		return
	}
	...
	if (await fileExistsAtPath(filePath)) {
		vscode.window.showErrorMessage(t("hoodycode:rules.errors.fileAlreadyExists", { filename }))
		return
	}
...
${t("hoodycode:rules.templates.workflow.description")}

${t("hoodycode:rules.templates.workflow.stepsHeader")}

1. ${t("hoodycode:rules.templates.workflow.step1")}
2. ${t("hoodycode:rules.templates.workflow.step2")}
`
}
...
${t("hoodycode:rules.templates.rule.description")}

${t("hoodycode:rules.templates.rule.guidelinesHeader")}

- ${t("hoodycode:rules.templates.rule.guideline1")}
- ${t("hoodycode:rules.templates.rule.guideline2")}
`
}

export async function deleteRuleFile(rulePath: string): Promise<void> {
	const deleteAction = t("hoodycode:rules.actions.delete")
	const filename = path.basename(rulePath)
	const result = await vscode.window.showWarningMessage(
		t("hoodycode:rules.actions.confirmDelete", { filename }),
		{ modal: true },
		deleteAction,
	)
	...
	if (result === deleteAction) {
		...
		vscode.window.showInformationMessage(t("hoodycode:rules.actions.deleted", { filename }))
	}
}
