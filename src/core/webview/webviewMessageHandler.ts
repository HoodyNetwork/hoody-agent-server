...
import { showSystemNotification } from "../../integrations/notifications" // hoodycode_change
import { singleCompletionHandler } from "../../utils/single-completion-handler" // hoodycode_change
...
import { toggleWorkflow, toggleRule, createRuleFile, deleteRuleFile } from "./hoodyrules"
import { mermaidFixPrompt } from "../prompts/utilities/mermaid" // hoodycode_change
import { editMessageHandler, fetchHoodycodeNotificationsHandler } from "../hoodycode/webview/webviewMessageHandlerUtils" // hoodycode_change
...
import { seeNewChanges } from "../checkpoints/hoodycode/seeNewChanges" // hoodycode_change
import { getTaskHistory } from "../../shared/hoodycode/getTaskHistory" // hoodycode_change
import { fetchAndRefreshOrganizationModesOnStartup, refreshOrganizationModes } from "./hoodyWebviewMessgeHandlerHelpers"
...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		case "webviewDidLaunch":
			...
			// hoodycode_change start: Fetch organization modes on startup
			...
			const { refreshWorkflowToggles } = await import("../context/instructions/workflows") // hoodycode_change
			await refreshWorkflowToggles(provider.context, provider.cwd) // hoodycode_change

			provider.postStateToWebview()
			provider.postRulesDataToWebview() // hoodycode_change: send workflows and rules immediately
			...
			provider.getStateToPostToWebview().then(async (/*hoodycode_change*/ state) => {
				const { telemetrySetting } = state
				const isOptedIn = telemetrySetting !== "disabled"
				TelemetryService.instance.updateTelemetryState(isOptedIn)
				await TelemetryService.instance.updateIdentity(state.apiConfiguration.hoodycodeToken ?? "") // hoodycode_change
			})
		...
		case "requestRouterModels":
			...
			const routerModels: Record<RouterName, ModelRecord> = {
				...
				huggingface: {},
				litellm: {},
				"hoodycode-openrouter": {}, // hoodycode_change
				deepinfra: {},
				"io-intelligence": {},
				requesty: {},
				unbound: {},
				glama: {},
				chutes: {}, // hoodycode_change
				ollama: {},
				lmstudio: {},
				ovhcloud: {}, // hoodycode_change
			}
			...
			// hoodycode_change start: openrouter auth, hoodycode provider
			const openRouterApiKey = apiConfiguration.openRouterApiKey || message?.values?.openRouterApiKey
			// Only use custom baseUrl if explicitly provided in the request message
			// This prevents using the Hoody baseUrl when fetching standard OpenRouter models
			const openRouterBaseUrl = message?.values?.openRouterBaseUrl

			const modelFetchPromises: Array<{ key: RouterName; options: GetModelsOptions }> = [
				{
					key: "openrouter",
					// Don't pass baseUrl unless explicitly provided - let it default to openrouter.ai
					options: { provider: "openrouter", apiKey: openRouterApiKey, ...(openRouterBaseUrl ? { baseUrl: openRouterBaseUrl } : {}) },
				},
				{
					key: "requesty",
					options: {
				...
				{ key: "chutes", options: { provider: "chutes", apiKey: apiConfiguration.chutesApiKey } }, // hoodycode_change
				{
					key: "hoodycode-openrouter",
					options: {
						provider: "hoodycode-openrouter",
						hoodycodeToken: apiConfiguration.hoodycodeToken,
						hoodycodeOrganizationId: apiConfiguration.hoodycodeOrganizationId,
					},
				},
		...
		case "requestOllamaModels": {
			...
			try {
				...
				const ollamaModels = await getModels({
					...
					numCtx: ollamaApiConfig.ollamaNumCtx, // hoodycode_change
				})
		...
		case "forkCountsRequest": {
			const current = provider.getCurrentTask()
			const counts: Record<number, number> = {}
			if (current) {
				const cwd = current.cwd || provider.cwd
				for (const item of provider.getTaskHistory()) {
					if (item.sourceTaskId === current.taskId) {
						// Count forks for this task; restrict to same workspace if provided
						if (!item.workspace || item.workspace === cwd) {
							const ts = (item as any).forkFromMessageTs
							if (typeof ts === "number") {
			...
			await provider.postMessageToWebview({
				...
				values: { counts, taskId: current?.taskId },
			})
			break
		}
		// hoodycode_change end
		...
		case "openProjectMcpSettings": {
			...
			const rooDir = path.join(workspaceFolder, ".hoodycode")
		...
		case "enableMcpServerCreation":
			...
			await provider.postStateToWebview()
			break
		// hoodycode_change begin
		case "openGlobalKeybindings":
			vscode.commands.executeCommand("workbench.action.openGlobalKeybindings", message.text ?? "hoody-code.")
			break
		...
		case "playTts":
			if (message.text) {
				...
				// Build task context for TTS messages
				const buildTaskContext = async () => {
					if (!currentTask) return undefined
					const state = await provider.getState()
					const modelInfo = currentTask.api?.getModel()
					return {
		...
		case "maxReadFileLine":
			...
			break
		// hoodycode_change start
		case "hoodyCodeImageApiKey":
			await provider.contextProxy.setValue("hoodyCodeImageApiKey", message.text)
			await provider.postStateToWebview()
			break
		...
		case "enhancementApiConfigId":
			...
			await provider.postStateToWebview()
			break
		// hoodycode_change start - commitMessageApiConfigId
		case "commitMessageApiConfigId":
			await updateGlobalState("commitMessageApiConfigId", message.text)
			await provider.postStateToWebview()
			break
		// hoodycode_change end - commitMessageApiConfigId
		// hoodycode_change start - terminalCommandApiConfigId
		case "terminalCommandApiConfigId":
			await updateGlobalState("terminalCommandApiConfigId", message.text)
			await provider.postStateToWebview()
			break
		// hoodycode_change end - terminalCommandApiConfigId
		// hoodycode_change start - ghostServiceSettings
		case "ghostServiceSettings":
			...
			await provider.postStateToWebview()
			vscode.commands.executeCommand("hoody-code.ghost.reload")
			break
		// hoodycode_change end
		...
		case "enhancePrompt":
			if (message.text) {
				try {
					...
					TelemetryService.instance.captureException(error, { context: "enhance_prompt" }) // hoodycode_change
		...
		case "showFeedbackOptions": {
			...
			if (answer === githubIssuesText) {
				await vscode.env.openExternal(vscode.Uri.parse("https://github.com/HoodyNetwork/hoody-agent-server/issues"))
			} else if (answer === discordText) {
				await vscode.env.openExternal(vscode.Uri.parse("https://discord.social.hoody.com"))
			} else if (answer === customerSupport) {
				await vscode.env.openExternal(vscode.Uri.parse("https://hoody.com/support"))
			}
			break
		}
		// hoodycode_change end
		...
		case "upsertApiConfiguration":
			// hoodycode_change start: check for hoodycodeToken change to remove organizationId and fetch organization modes
			if (message.text && message.apiConfiguration) {
				...
				try {
					...
					// Only clear organization ID if we actually had a hoodycode token before and it's different now
					const hadPreviousToken = currentConfig.hoodycodeToken !== undefined
					const hasNewToken = message.apiConfiguration.hoodycodeToken !== undefined
					const tokensAreDifferent = currentConfig.hoodycodeToken !== message.apiConfiguration.hoodycodeToken

					if (hadPreviousToken && hasNewToken && tokensAreDifferent) {
						configToSave = { ...message.apiConfiguration, hoodycodeOrganizationId: undefined }
					}

					organizationChanged =
						currentConfig.hoodycodeOrganizationId !== message.apiConfiguration.hoodycodeOrganizationId
					...
					if (organizationChanged) {
						...
						await flushModels("hoodycode-openrouter")
						const models = await getModels({
							provider: "hoodycode-openrouter",
							hoodycodeOrganizationId: message.apiConfiguration.hoodycodeOrganizationId,
							hoodycodeToken: message.apiConfiguration.hoodycodeToken,
						})
						provider.postMessageToWebview({
							type: "routerModels",
							routerModels: { "hoodycode-openrouter": models } as Record<RouterName, ModelRecord>,
						})
					}
				} catch (error) {
			...
			// hoodycode_change end: check for hoodycodeToken change to remove organizationId and fetch organization modes
			break
		...
		case "forkTaskFromMessage": {
			const current = provider.getCurrentTask()
			if (!current) {
				await vscode.window.showErrorMessage("No active task to fork")
				break
			}
			const ts = message.messageTs
			if (typeof ts !== "number") {
				await vscode.window.showErrorMessage("Invalid message timestamp for fork")
				break
			}
			try {
				// Count how many UI messages will be included (inclusive)
				const messagesIncluded = Array.isArray(current.clineMessages)
					? current.clineMessages.filter((m: any) => typeof m?.ts === "number" && (m.ts as number) <= ts).length
					: 0

				await provider.forkTaskFromMessage(current.taskId, ts, { makeActive: true })

				// Inform the user; state will refresh as provider switches tasks
				vscode.window.showInformationMessage(`Fork created (${messagesIncluded} messages)`)
			} catch (err) {
				const errorMessage = err instanceof Error ? err.message : String(err)
				if (/partial|streaming/i.test(errorMessage)) {
					await vscode.window.showErrorMessage("Cannot fork from partial or streaming message")
				} else {
					await vscode.window.showErrorMessage(`Failed to fork: ${errorMessage}`)
				}
			}
			break
		}
		...
		// hoodycode_change_start
		case "fetchProfileDataRequest":
			try {
				const { apiConfiguration, currentApiConfigName } = await provider.getState()
				const hoodycodeToken = apiConfiguration?.hoodycodeToken

				if (!hoodycodeToken) {
					provider.log("HoodyCode token not found in extension state.")
					provider.postMessageToWebview({
						type: "profileDataResponse",
						payload: { success: false, error: "HoodyCode API token not configured." },
					})
					break
				}
				...
				if (
					apiConfiguration.hoodycodeTesterWarningsDisabledUntil &&
					apiConfiguration.hoodycodeTesterWarningsDisabledUntil > Date.now()
				) {
					headers["X-HOODYCODE-TESTER"] = "SUPPRESS"
				}

				const response = await axios.get<Omit<ProfileData, "hoodycodeToken">>(
					`${getHoodyBaseUriFromToken(hoodycodeToken)}/api/profile`,
					{
						headers,
					},
				)
				...
				const organizationExists = (response.data.organizations ?? []).some(
					({ id }) => id === apiConfiguration?.hoodycodeOrganizationId,
				)
				if (apiConfiguration?.hoodycodeOrganizationId && !organizationExists) {
					provider.upsertProviderProfile(currentApiConfigName ?? "default", {
						...apiConfiguration,
						hoodycodeOrganizationId: undefined,
					})
				}

				provider.postMessageToWebview({
					type: "profileDataResponse", // Assuming this response type is still appropriate for /api/profile
					payload: { success: true, data: { hoodycodeToken, ...response.data } },
				})
			} catch (error: any) {
				const errorMessage =
		...
		case "fetchBalanceDataRequest": // New handler
			try {
				const { apiConfiguration } = await provider.getState()
				const { hoodycodeToken, hoodycodeOrganizationId } = apiConfiguration ?? {}

				if (!hoodycodeToken) {
					provider.log("HoodyCode token not found in extension state for balance data.")
					provider.postMessageToWebview({
				...
				if (hoodycodeOrganizationId) {
					headers["X-HoodyCode-OrganizationId"] = hoodycodeOrganizationId
				}
				...
				const response = await axios.get(`${getHoodyBaseUriFromToken(hoodycodeToken)}/api/profile/balance`, {
		...
		case "shopBuyCredits": // New handler
			try {
				...
				const hoodycodeToken = apiConfiguration?.hoodycodeToken
				if (!hoodycodeToken) {
					provider.log("HoodyCode token not found in extension state for buy credits.")
					break
				}
				...
				const baseUrl = getHoodyBaseUriFromToken(hoodycodeToken)
		...
		case "createRuleFile": {
			if (
				...
				try {
					...
					vscode.window.showErrorMessage(t("hoodycode:rules.errors.failedToCreateRuleFile"))
				}
				await provider.postRulesDataToWebview()
			}
			break
		...
		case "deleteRuleFile": {
			if (message.rulePath) {
				try {
					...
					vscode.window.showErrorMessage(t("hoodycode:rules.errors.failedToDeleteRuleFile"))
				}
				await provider.postRulesDataToWebview()
			}
			break
		...
		// hoodycode_change start - add clearUsageData
		case "clearUsageData": {
			try {
		...
		// hoodycode_change start - add getUsageData
		case "getUsageData": {
			if (message.text) {
				try {
					const usageTracker = UsageTracker.getInstance()
		...
		// hoodycode_change end - add getUsageData
		// hoodycode_change start - add toggleTaskFavorite
		case "toggleTaskFavorite":
			if (message.text) {
				await provider.toggleTaskFavorite(message.text)
			}
			break
		// hoodycode_change start - add fixMermaidSyntax
		...
		case "fetchHoodycodeNotifications": {
			await fetchHoodycodeNotificationsHandler(provider)
			break
		}
