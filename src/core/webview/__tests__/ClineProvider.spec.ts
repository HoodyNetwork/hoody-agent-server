...
} from "@roo-code/types" // hoodycode_change: openRouterDefaultModelId
...
vi.mock("vscode", () => ({
	...
	window: {
		...
		createTextEditorDecorationType: vi.fn(() => ({ dispose: vi.fn() })), // hoodycode_change
	},
	workspace: {
	...
	env: {
		uriScheme: "vscode",
		language: "en",
		uiKind: 1, // hoodycode_change Desktop
...
describe("ClineProvider", () => {
	...
	test("resolveWebviewView sets up webview correctly in development mode even if local server is not running", async () => {
		...
		expect(mockWebviewView.webview.html).toContain(
			// hoodycode_change: added localhost:3000
	...
	test("postMessageToWebview sends message to webview", async () => {
		...
		const mockState: ExtensionState = {
			...
			apiConfiguration: {
				...
				hoodycodeModel: openRouterDefaultModelId,
				hoodycodeToken: "hoodycode-token",
				// hoodycode_change end
			},
	...
	test("getState returns correct initial state", async () => {
		...
		// expect(state).toHaveProperty("taskHistory") // hoodycode_change
...
describe("Project MCP Settings", () => {
	...
	test.skip("handles openProjectMcpSettings message", async () => {
		...
		expect(mockedFs.mkdir).toHaveBeenCalledWith("/test/workspace/.hoodycode", { recursive: true })

		// Verify file was created with default content
		expect(safeWriteJson).toHaveBeenCalledWith("/test/workspace/.roo/mcp.json", { mcpServers: {} })

		// Check that openFile was called
		expect(openFileSpy).toHaveBeenCalledWith("/test/workspace/.hoodycode/mcp.json")
	})
	...
	test.skip("handles openProjectMcpSettings file creation error", async () => {
		...
		expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
			// hoodycode_change
			expect.stringContaining("Failed to create or open .hoodycode/mcp.json"),
		)
	})
})
...
describe("ClineProvider - Router Models", () => {
	...
	test("handles requestRouterModels with successful responses", async () => {
		...
		vi.spyOn(provider, "getState").mockResolvedValue({
			apiConfiguration: {
				...
				chutesApiKey: "chutes-key", // hoodycode_change
				litellmApiKey: "litellm-key",
				litellmBaseUrl: "http://localhost:4000",
				ovhCloudAiEndpointsApiKey: "ovhcloud-key", // hoodycode_change
			},
		} as any)
		...
		expect(getModels).toHaveBeenCalledWith({ provider: "openrouter", apiKey: "openrouter-key" }) // hoodycode_change: apiKey
		...
		expect(getModels).toHaveBeenCalledWith({ provider: "chutes", apiKey: "chutes-key" }) // hoodycode_change
		expect(getModels).toHaveBeenCalledWith({ provider: "vercel-ai-gateway" })
		expect(getModels).toHaveBeenCalledWith({ provider: "ovhcloud", apiKey: "ovhcloud-key" }) // hoodycode_change
		...
		expect(mockPostMessage).toHaveBeenCalledWith({
			...
			routerModels: {
				...
				glama: mockModels,
				unbound: mockModels,
				chutes: mockModels, // hoodycode_change
				litellm: mockModels,
				"hoodycode-openrouter": mockModels,
				ollama: mockModels, // hoodycode_change
				lmstudio: {},
				"vercel-ai-gateway": mockModels,
				ovhcloud: mockModels, // hoodycode_change
				huggingface: {},
				"io-intelligence": {},
			},
		})
	...
	test("handles requestRouterModels with individual provider failures", async () => {
		...
		vi.mocked(getModels)
			...
			.mockRejectedValueOnce(new Error("Chutes API error")) // hoodycode_change: chutes fail
			.mockRejectedValueOnce(new Error("Hoodycode-OpenRouter API error")) // hoodycode-openrouter fail
			.mockRejectedValueOnce(new Error("Ollama API error")) // hoodycode_change
			.mockResolvedValueOnce(mockModels) // vercel-ai-gateway success
			.mockResolvedValueOnce(mockModels) // hoodycode_change: ovhcloud
		...
		expect(mockPostMessage).toHaveBeenCalledWith({
			type: "singleRouterModelFetchResponse",
			success: false,
			error: "Hoodycode-OpenRouter API error",
			values: { provider: "hoodycode-openrouter" },
		})
