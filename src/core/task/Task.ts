...
import { processHoodyUserContentMentions } from "../mentions/processHoodyUserContentMentions" // hoodycode_change
import { refreshWorkflowToggles } from "../context/instructions/workflows" // hoodycode_change
import { parseMentions } from "../mentions" // hoodycode_change
import { parseHoodySlashCommands } from "../slash-commands/hoody" // hoodycode_change
import { GlobalFileNames } from "../../shared/globalFileNames" // hoodycode_change
import { ensureLocalHoodyrulesDirExists } from "../context/instructions/hoody-rules" // hoodycode_change
import { getMessagesSinceLastSummary, summarizeConversation } from "../condense"
import { Gpt5Metadata, ClineMessageWithMetadata } from "./types"
import { MessageQueueService } from "../message-queue/MessageQueueService"

import { AutoApprovalHandler } from "./AutoApprovalHandler"
import { isAnyRecognizedHoodyCodeError, isPaymentRequiredError } from "../../shared/hoodycode/errorUtils"
...
export interface TaskOptions extends CreateTaskOptions {
	context: vscode.ExtensionContext // hoodycode_change
...
type UserContent = Array<Anthropic.ContentBlockParam> // hoodycode_change

export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	private context: vscode.ExtensionContext // hoodycode_change

	readonly taskId: string
	private taskIsFavorited?: boolean // hoodycode_change
	...
	constructor({
		...
		super()
		this.context = context // hoodycode_change
		...
		this.taskIsFavorited = historyItem?.isFavorited // hoodycode_change
		...
		this.workspacePath = parentTask
			...
			: (workspacePath ?? getWorkspacePath(path.join(os.homedir(), "Documents"))) // hoodycode_change: use Documents instead of Desktop as default
	...
	/**
	 * Build general metadata for a message
	 *
	 * Constructs metadata containing model, profile, mode, and optionally token/cost information
	 * for a specific message. This metadata helps external UIs track per-message metrics.
	 *
	 * @param includeMetrics - Whether to include token/cost metrics (default: false)
	 * @param metrics - Optional metrics to include (tokens, cost)
	 * @returns Metadata object with general information
	 *
	 * Related files:
	 * - {@link packages/types/src/message.ts} - Zod schema definition
	 * - {@link src/services/api-server/controllers/TasksController.ts} - API exposure
	 */
	private async buildMessageMetadata(
		...
		try {
			...
			const general: any = {
				...
				profileId: state?.listApiConfigMeta?.find((p: any) => p.name === state?.currentApiConfigName)?.id,
				mode: this._taskMode || defaultModeSlug,
			}

			// Add metrics if requested and available
			if (includeMetrics && metrics) {
				if (metrics.tokensIn !== undefined) general.tokensIn = metrics.tokensIn
				if (metrics.tokensOut !== undefined) general.tokensOut = metrics.tokensOut
				if (metrics.cacheWrites !== undefined) general.cacheWrites = metrics.cacheWrites
				if (metrics.cacheReads !== undefined) general.cacheReads = metrics.cacheReads
				if (metrics.cost !== undefined) general.cost = metrics.cost

				// Calculate cumulative metrics from all previous messages
				const tokenUsage = this.getTokenUsage()
				general.cumulativeCost = tokenUsage.totalCost
				general.cumulativeTokensIn = tokenUsage.totalTokensIn
				general.cumulativeTokensOut = tokenUsage.totalTokensOut
			}

			return { general }
		} catch (error) {
			// Non-fatal - if we can't build metadata, just return empty
	...
	private async addToClineMessages(message: ClineMessage) {
		...
		// hoodycode_change start: no cloud service
		...
		// Agent Digest: Auto-generate digest if enabled
		// This runs asynchronously in background to avoid blocking the task
		if (!message.partial && this.shouldAutoDigest() && this.isDigestEligible(message)) {
			this.generateMessageDigest(message).catch((error) => {
				// Digest generation is non-critical, just log and continue
				console.warn(`[Task#${this.taskId}] Failed to auto-generate digest:`, error)
			})
		}
	}

	/**
	 * Check if auto-digest should be generated for messages in this task
	 */
	private shouldAutoDigest(): boolean {
		const provider = this.providerRef.deref()
		if (!provider) return false

		try {
			const digestEnabled = provider.contextProxy.getValue("digestEnabled")
			const autoDigestEnabled = provider.contextProxy.getValue("autoDigestEnabled")

			if (!digestEnabled || autoDigestEnabled === false) {
				return false
			}

			// Check task-specific override
			const taskOverrides = provider.contextProxy.getValue("digestTaskOverrides") || {}
			const taskOverride = taskOverrides[this.taskId]
			...
			console.error("[Task#shouldAutoDigest] Error checking digest settings:", error)
			return false
		}
	}

	/**
	 * Determine if a message is eligible for digest generation (scope filter).
	 * We only digest:
	 * - Assistant replies: say === "text"
	 * - Tool invocations/results represented as ask === "tool"
	 */
	private isDigestEligible(message: ClineMessage): boolean {
		try {
			if (message.partial) return false
			if (message.type === "say" && message.say === "text") return true
			if (message.type === "ask" && message.ask === "tool") return true
			return false
		} catch {
			return false
		}
	...
	/**
	 * Generate digest for a message (async, non-blocking)
	 */
	private async generateMessageDigest(message: ClineMessage): Promise<void> {
		try {
			const provider = this.providerRef.deref()
			if (!provider) return

			// Import DigestService
			const { DigestService } = await import("../../services/digest/DigestService")
			const digestService = new DigestService(provider.contextProxy, provider)

			// Get task-specific settings
			const settings = digestService.getTaskSettings(this.taskId)
			...
			// Save updated messages (includes the new digest)
			await this.saveClineMessages()

			// Broadcast update to WebSocket clients
			await this.updateClineMessage(message)

			console.log(`[Task#${this.taskId}] Auto-generated digest: "${result.digest}" (${result.digest.length} chars)`)
		} catch (error) {
			// Don't throw - digest generation is non-critical
			console.error(`[Task#${this.taskId}] Digest generation failed:`, error)
		}
	}
	...
	private async updateClineMessage(message: ClineMessage) {
		...
		// Build task context for remote clients
		const state = await provider?.getState()
		const modelInfo = this.api?.getModel()
		const taskContext = {
			mode: this._taskMode || (await this.getTaskMode()),
			modelId: modelInfo?.id || state?.apiConfiguration.apiModelId,
	...
	private async saveClineMessages() {
		try {
			...
			// Get current context information for history
			const provider = this.providerRef.deref()
			const state = await provider?.getState()
			const modelInfo = this.api?.getModel()
	...
	async ask(
		...
		if (this.abort) {
			throw new Error(`[HoodyCode#ask] task ${this.taskId}.${this.instanceId} aborted`)
		}

		// Build general metadata for this message
		const generalMetadata = await this.buildMessageMetadata()
		...
		if (partial !== undefined) {
			...
			if (partial) {
				if (isUpdatingPreviousPartial) {
					...
					await this.addToClineMessages({ ts: askTs, type: "ask", ask: type, text, partial, isProtected, metadata: generalMetadata })
				...
				if (isUpdatingPreviousPartial) {
					...
					askTs = Date.now()
					this.lastMessageTs = askTs
					await this.addToClineMessages({ ts: askTs, type: "ask", ask: type, text, isProtected, metadata: generalMetadata })
				}
			}
		} else {
	...
	handleWebviewAskResponse(askResponse: ClineAskResponse, text?: string, images?: string[]) {
		// this.askResponse = askResponse hoodycode_change
	...
	async say(
		...
		if (this.abort) {
			throw new Error(`[Hoody Code#say] task ${this.taskId}.${this.instanceId} aborted`)
		}
			...
			if (partial) {
				if (isUpdatingPreviousPartial) {
					...
					await this.addToClineMessages({
						...
						partial,
						contextCondense,
						metadata: { ...options.metadata, ...generalMetadata },
					})
				}
			} else {
				...
				if (isUpdatingPreviousPartial) {
					...
					lastMessage.partial = false
					lastMessage.progressStatus = progressStatus
					if (options.metadata || generalMetadata) {
						...
						Object.assign(messageWithMetadata.metadata, options.metadata, generalMetadata)
					}
	...
	async sayAndCreateMissingParamError(toolName: ToolName, paramName: string, relPath?: string) {
		const hoodycodeExtraText = (() => {
			switch (toolName) {
				case "apply_diff":
					return t("hoodycode:task.disableApplyDiff") + " "
				case "edit_file":
					return t("hoodycode:task.disableEditFile") + " "
				default:
					return ""
			}
		})()
		await this.say(
			"error",
			`Hoody Code tried to use ${toolName}${relPath ? ` for '${relPath.toPosix()}'` : ""
			} without value for required parameter '${paramName}'. ${hoodycodeExtraText}Retrying...`,
		)
	...
	public async recursivelyMakeClineRequests(
		...
		while (stack.length > 0) {
			...
			if (this.abort) {
				throw new Error(
					`[HoodyCode#recursivelyMakeClineRequests] task ${this.taskId}.${this.instanceId} aborted`,
				)
			}
			...
			// hoodycode_change start
			const [parsedUserContent, needsRulesFileCheck] = await processHoodyUserContentMentions({
			...
			if (needsRulesFileCheck) {
				await this.say(
					"error",
					"Issue with processing the /newrule command. Double check that, if '.hoodycode/rules' already exists, it's a directory and not a file. Otherwise there was an issue referencing this file/directory",
				)
			}
			// hoodycode_change end
			...
			try {
				...
				const updateApiReqMsg = (cancelReason?: ClineApiReqCancelReason, streamingFailedMessage?: string) => {
					...
					const calculatedCost = totalCost ??
					...
					// Also populate metadata.general with per-message metrics
					const apiReqMessage = this.clineMessages[lastApiReqIndex]
					if (apiReqMessage && !apiReqMessage.metadata) {
						apiReqMessage.metadata = {}
					}
					if (apiReqMessage && !apiReqMessage.metadata!.general) {
						apiReqMessage.metadata!.general = {}
					}
					if (apiReqMessage && apiReqMessage.metadata!.general) {
						// Add per-message metrics
						apiReqMessage.metadata!.general.tokensIn = inputTokens
						apiReqMessage.metadata!.general.tokensOut = outputTokens
						apiReqMessage.metadata!.general.cacheWrites = cacheWriteTokens
						apiReqMessage.metadata!.general.cacheReads = cacheReadTokens
						apiReqMessage.metadata!.general.cost = calculatedCost

						// Calculate cumulative metrics
						const tokenUsage = this.getTokenUsage()
						apiReqMessage.metadata!.general.cumulativeCost = tokenUsage.totalCost
						apiReqMessage.metadata!.general.cumulativeTokensIn = tokenUsage.totalTokensIn
						apiReqMessage.metadata!.general.cumulativeTokensOut = tokenUsage.totalTokensOut
					}
				}
					...
					while (!item.done) {
						...
						switch (chunk.type) {
							...
							case "usage":
								...
								inferenceProvider = chunk.inferenceProvider // hoodycode_change
								break
							case "grounding":
				...
				if (assistantMessage.length > 0) {
					...
					await this.say(
						"error",
						t("hoodycode:task.noAssistantMessages"), // hoodycode_change
					)
	...
	async loadContext(
				...
				userContent.map(async (block) => {
					if (block.type === "text") {
						...
						if (
							...
							const { processedText, needsRulesFileCheck: needsCheck } = await parseHoodySlashCommands(
								parsedText,
		...
		if (needsClinerulesFileCheck) {
			clinerulesError = await ensureLocalHoodyrulesDirExists(this.cwd, GlobalFileNames.hoodyRules)
		}
	...
	/*private hoodycode_change*/ async getSystemPrompt(): Promise<string> {
		...
		return await (async () => {
			...
			return SYSTEM_PROMPT(
				provider.context,
				this.cwd,
				// hoodycode_change: supports images => supports browser
				...
				maxReadFileLine !== -1,
				{
					...
					useAgentRules: vscode.workspace.getConfiguration("hoody-code").get<boolean>("useAgentRules") ?? true,
					newTaskRequireTodos: vscode.workspace
						.getConfiguration("hoody-code")
						.get<boolean>("newTaskRequireTodos", false),
				},
				undefined, // todoList
				this.api.getModel().id,
				await provider.getState(), // hoodycode_change
			)
		})()
	}
	...
	public async *attemptApiRequest(retryAttempt: number = 0): ApiStream {
		...
		if (Task.lastGlobalApiRequestTime) {
			const now = performance.now() // hoodycode_change
		...
		Task.lastGlobalApiRequestTime = performance.now() // hoodycode_change
		...
		try {
			...
			this.isWaitingForFirstChunk = false
			// hoodycode_change start
			if (apiConfiguration?.apiProvider === "hoodycode" && isAnyRecognizedHoodyCodeError(error)) {
				const { response } = await (isPaymentRequiredError(error)
					? this.ask(
						"payment_required_prompt",
						JSON.stringify({
							title: error.error?.title ?? t("hoodycode:lowCreditWarning.title"),
							message: error.error?.message ?? t("hoodycode:lowCreditWarning.message"),
							balance: error.error?.balance ?? "0.00",
							buyCreditsUrl: error.error?.buyCreditsUrl ?? "https://hoody.com/profile",
						}),
					)
					: this.ask(
						"invalid_model",
						JSON.stringify({
							modelId: apiConfiguration.hoodycodeModel,
							error: {
								status: error.status,
	...
	public recordToolError(toolName: ToolName, error?: string) {
		...
		TelemetryService.instance.captureEvent(TelemetryEventName.TOOL_ERROR, { toolName, error }) // hoodycode_change
	}
