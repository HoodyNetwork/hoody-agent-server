...
import { HoodycodeOpenrouterHandler } from "../../api/providers/hoodycode-openrouter"
import { fileWriteQueue } from "../../utils/FileWriteQueue"

// Hardcoded list of image generation models for now
const IMAGE_GENERATION_MODELS = ["google/gemini-2.5-flash-image"] // hoodycode_change
...
export async function generateImageTool(
	...
	// hoodycode_change: Use hoodyCodeImageApiKey if set, otherwise fall back to hoodycodeToken
	const hoodyCodeApiKey = state?.hoodyCodeImageApiKey || state?.apiConfiguration?.hoodycodeToken

	if (!openRouterApiKey && !hoodyCodeApiKey) {
		await cline.say(
			"error",
	...
	try {
		if (!block.partial) {
			...
			const openRouterHandler = openRouterApiKey
				...
				: new HoodycodeOpenrouterHandler({
					hoodycodeToken: hoodyCodeApiKey,
					hoodycodeOrganizationId:
						cline.apiConfiguration.apiProvider === "hoodycode" &&
							cline.apiConfiguration.hoodycodeToken === hoodyCodeApiKey
							? cline.apiConfiguration.hoodycodeOrganizationId
							: undefined,
				})
			...
			// Write the image file (using queue for concurrency protection)
			await fileWriteQueue.writeFile(absolutePath, imageBuffer)
