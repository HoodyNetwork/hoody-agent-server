...
import { t } from "../../i18n"
import { McpToolCallResponse } from "../../shared/mcp" // hoodycode_change
import { summarizeSuccessfulMcpOutputWhenTooLong } from "./hoodycode" // hoodycode_change
...
// hoodycode_change: make async, add task parameter
async function processToolContent(task: Task, toolResult: McpToolCallResponse): Promise<string> {
	if (!toolResult?.content || toolResult.content.length === 0) {
		return ""
	}

	const outputText = toolResult.content // hoodycode_change: introduce const
		.map((item: any) => {
			if (item.type === "text") {
				return item.text
			}
...
async function executeToolAndProcessResult(
	...
	if (toolResult) {
		// hoodycode_change: await, add api parameter
