...
export async function reportBugTool(
	...
	try {
		if (block.partial) {
			...
			const hoodycodeVersion =
				vscode.extensions.getExtension("hoodycode.hoody-code")?.packageJSON.version || "Unknown"
			...
			const bugReportData = JSON.stringify({
				...
				hoodycode_version: hoodycodeVersion,
			})
			...
			if (text || images?.length) {
				...
				try {
					...
					await createAndOpenGitHubIssue("Hoody-Org", "hoodycode", "bug_report.yml", params)
				} catch (error) {
