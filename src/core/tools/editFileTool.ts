// hoodycode_change: Fast Apply -- file added
...
import { X_HOODYCODE_ORGANIZATIONID, X_HOODYCODE_TASKID, X_HOODYCODE_TESTER } from "../../shared/hoodycode/headers"
...
async function applyFastApplyEdit(
	...
	try {
		...
		const hoodyTesterSuppressUntil = state.apiConfiguration.hoodycodeTesterWarningsDisabledUntil
		const hoodyTesterSuppress =
			hoodyTesterSuppressUntil && hoodyTesterSuppressUntil > Date.now() ? { [X_HOODYCODE_TESTER]: "SUPPRESS" } : {}
		...
		const client = new OpenAI({
			...
			defaultHeaders: {
				...DEFAULT_HEADERS,
				...(morphConfig.hoodyCodeOrganizationId
					? { [X_HOODYCODE_ORGANIZATIONID]: morphConfig.hoodyCodeOrganizationId }
					: {}),
				...hoodyTesterSuppress,
				[X_HOODYCODE_TASKID]: cline.taskId,
			},
		})
...
interface FastApplyConfiguration {
	...
	model?: string
	error?: string
	hoodyCodeOrganizationId?: string
}
...
function getFastApplyConfiguration(state: ClineProviderState): FastApplyConfiguration {
	...
	// Priority 2: Use HoodyCode provider
	if (state.apiConfiguration?.apiProvider === "hoodycode") {
		const token = state.apiConfiguration.hoodycodeToken
		if (!token) {
			return { available: false, error: "No HoodyCode token available to use Fast Apply" }
		}
		return {
			available: true,
			apiKey: token,
			baseUrl: `${getHoodyBaseUriFromToken(token)}/api/openrouter/`,
			model: selectedModel === "auto" ? "morph/morph-v3-large" : selectedModel, // Use selected model
			hoodyCodeOrganizationId: state.apiConfiguration.hoodycodeOrganizationId,
		}
	}
