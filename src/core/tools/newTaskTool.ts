...
export async function newTaskTool(
	...
	try {
		if (block.partial) {
			...
			// hoodycode_change start: Switch to the desired mode BEFORE creating the task
			...
			if (!newTask) {
				await provider.handleModeSwitch(task.pausedModeSlug) // hoodycode_change: if task creation failed, switch back to the parent's mode
