...
export async function getCheckpointService(
	...
	try {
		...
		task.enableCheckpoints = false
		reportError("Task#getCheckpointService", err) // hoodycode_change
	...
	try {
		...
		service.on("checkpoint", ({ fromHash: from, toHash: to, suppressMessage }) => {
			try {
				...
				buildTaskContext().then(taskContext => {
				...
				task.say(
					...
					console.error(err)
					reportError("getCheckpointService:say('checkpoint_saved')", err) // hoodycode_change
				})
			} catch (err) {
				log("[Task#getCheckpointService] caught unexpected error in on('checkpoint'), disabling checkpoints")
				console.error(err)
				task.enableCheckpoints = false
				reportError("getCheckpointService:on('checkpoint')", err) // hoodycode_change
			}
		})
		...
		try {
			...
			task.enableCheckpoints = false
			reportError("getCheckpointService:initShadowGit", err) // hoodycode_change
		}
	} catch (err) {
		...
		task.enableCheckpoints = false
		task.checkpointServiceInitializing = false
		reportError("getCheckpointService", err) // hoodycode_change
	}
}
...
export async function checkpointSave(task: Task, force = false, suppressMessage = false) {
	...
	return service
		...
		.catch((err) => {
			...
			task.enableCheckpoints = false
			reportError("checkpointSave", err) // hoodycode_change
		})
}
...
export async function checkpointRestore(
	...
	try {
		...
		task.enableCheckpoints = false
		reportError("checkpointRestore", err) // hoodycode_change
	}
}
...
export async function checkpointDiff(task: Task, { ts, previousCommitHash, commitHash, mode }: CheckpointDiffOptions) {
	...
	try {
		...
		task.enableCheckpoints = false
		reportError("checkpointDiff", err) // hoodycode_change
	}
}
