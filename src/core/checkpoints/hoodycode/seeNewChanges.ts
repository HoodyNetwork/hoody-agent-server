...
export async function seeNewChanges(task: Task, commitRange: CommitRange) {
	try {
		const service = await getCheckpointService(task)
		if (!service) {
			vscode.window.showWarningMessage(t("hoodycode:seeNewChanges.checkpointsUnavailable"))
			return
		}

		const changes = await service.getDiff(commitRange)
		if (changes.length === 0) {
			vscode.window.showWarningMessage(t("hoodycode:seeNewChanges.noChanges"))
			return
		}

		await vscode.commands.executeCommand(
			"vscode.changes",
			t("hoodycode:seeNewChanges.title"),
			changes.map((change) => [
		...
		vscode.window.showErrorMessage(t("hoodycode:seeNewChanges.error"))
