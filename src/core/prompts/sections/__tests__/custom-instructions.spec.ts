...
describe("loadRuleFiles", () => {
	...
	it("should not combine content from multiple rule files when they exist", async () => {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			if (filePath.toString().endsWith(".hoodycoderules")) {
	...
	it("should use .hoodycode/rules/ directory when it exists and has files", async () => {
		// Simulate .hoodycode/rules directory exists
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				parentPath: "/fake/path/.hoodycode/rules",
			},
			{
				name: "file2.txt",
				isFile: () => true,
		...
		statMock.mockImplementation((path) => {
			...
			if (
				normalizedPath.includes("/fake/path/.hoodycode/rules/file1.txt") ||
				normalizedPath.includes("/fake/path/.hoodycode/rules/file2.txt")
			) {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.hoodycode/rules/file1.txt") {
				return Promise.resolve("content of file1")
			}
			if (normalizedPath === "/fake/path/.hoodycode/rules/file2.txt") {
		...
		const expectedPath1 =
			process.platform === "win32"
				? "\\fake\\path\\.hoodycode\\rules\\file1.txt"
				: "/fake/path/.hoodycode/rules/file1.txt"
		const expectedPath2 =
			process.platform === "win32"
				? "\\fake\\path\\.hoodycode\\rules\\file2.txt"
				: "/fake/path/.hoodycode/rules/file2.txt"
		...
		const expectedRulesDir =
			process.platform === "win32" ? "\\fake\\path\\.hoodycode\\rules" : "/fake/path/.hoodycode/rules"
	...
	it("should fall back to .hoodycoderules when .hoodycode/rules/ is empty", async () => {
		// Simulate .hoodycode/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		// Simulate .hoodycoderules exists
		readFileMock.mockImplementation((filePath: PathLike) => {
			if (filePath.toString().endsWith(".hoodycoderules")) {
				return Promise.resolve("roo rules content")
			}
	...
	it("should read files from nested subdirectories in .hoodycode/rules/", async () => {
		// Simulate .hoodycode/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			...
			},
			{
				...
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.hoodycode/rules/subdir",
			},
			{
				name: "nested2.txt",
				isFile: () => true,
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.hoodycode/rules/subdir/subdir2",
			},
		] as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.hoodycode/rules/root.txt") {
				return Promise.resolve("root file content")
			}
			if (normalizedPath === "/fake/path/.hoodycode/rules/subdir/nested1.txt") {
				return Promise.resolve("nested file 1 content")
			}
			if (normalizedPath === "/fake/path/.hoodycode/rules/subdir/subdir2/nested2.txt") {
		...
		const expectedRootPath =
			process.platform === "win32"
				? "\\fake\\path\\.hoodycode\\rules\\root.txt"
				: "/fake/path/.hoodycode/rules/root.txt"
		const expectedNested1Path =
			process.platform === "win32"
				? "\\fake\\path\\.hoodycode\\rules\\subdir\\nested1.txt"
				: "/fake/path/.hoodycode/rules/subdir/nested1.txt"
		const expectedNested2Path =
			process.platform === "win32"
				? "\\fake\\path\\.hoodycode\\rules\\subdir\\subdir2\\nested2.txt"
				: "/fake/path/.hoodycode/rules/subdir/subdir2/nested2.txt"
...
describe("addCustomInstructions", () => {
	...
	it("should combine all instruction types when provided", async () => {
		// Simulate no .hoodycode/rules-test-mode directory
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
		...
		expect(result).toContain("Rules from .hoodycoderules-test-mode:\nmode specific rules")
	})
	...
	it("should use .hoodycode/rules-test-mode/ directory when it exists and has files", async () => {
		// Simulate .hoodycode/rules-test-mode directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				isFile: () => true,
				isSymbolicLink: () => false,
				parentPath: "/fake/path/.hoodycode/rules-test-mode",
			},
			{
				name: "rule2.txt",
				isFile: () => true,
		...
		statMock.mockImplementation((path) => {
			...
			const normalizedPath = path.toString().replace(/\\/g, "/")
			if (
				normalizedPath.includes("/fake/path/.hoodycode/rules-test-mode/rule1.txt") ||
				normalizedPath.includes("/fake/path/.hoodycode/rules-test-mode/rule2.txt")
			) {
				return Promise.resolve({
					isFile: vi.fn().mockReturnValue(true),
				}) as any
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.hoodycode/rules-test-mode/rule1.txt") {
				return Promise.resolve("mode specific rule 1")
			}
			if (normalizedPath === "/fake/path/.hoodycode/rules-test-mode/rule2.txt") {
		...
		const expectedTestModeDir =
			process.platform === "win32"
				? "\\fake\\path\\.hoodycode\\rules-test-mode"
				: "/fake/path/.hoodycode/rules-test-mode"
		const expectedRule1Path =
			process.platform === "win32"
				? "\\fake\\path\\.hoodycode\\rules-test-mode\\rule1.txt"
				: "/fake/path/.hoodycode/rules-test-mode/rule1.txt"
		const expectedRule2Path =
			process.platform === "win32"
				? "\\fake\\path\\.hoodycode\\rules-test-mode\\rule2.txt"
				: "/fake/path/.hoodycode/rules-test-mode/rule2.txt"
	...
	it("should fall back to .hoodycoderules-test-mode when .hoodycode/rules-test-mode/ does not exist", async () => {
		// Simulate .hoodycode/rules-test-mode directory does not exist
		statMock.mockRejectedValueOnce({ code: "ENOENT" })

		// Simulate .hoodycoderules-test-mode exists
		readFileMock.mockImplementation((filePath: PathLike) => {
			if (filePath.toString().includes(".hoodycoderules-test-mode")) {
	...
	it("should correctly format content from directories when using .hoodycode/rules-test-mode/", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "rule1.txt", isFile: () => true, parentPath: "/fake/path/.hoodycode/rules-test-mode" },
		] as any)
		readFileMock.mockReset()
...
describe("Rules directory reading", () => {
	it.skipIf(process.platform === "win32")("should follow symbolic links in the rules directory", async () => {
			...
			.mockResolvedValueOnce([
				{
					...
					isFile: () => true,
					parentPath: "/fake/path/.hoodycode/rules/symlink-target-dir",
				},
			] as any)
		...
		statMock.mockImplementation((path: string) => {
			// For directory check
			if (path === "/fake/path/.hoodycode/rules" || path.endsWith("dir")) {
				return Promise.resolve({
					isDirectory: vi.fn().mockReturnValue(true),
					isFile: vi.fn().mockReturnValue(false),
				} as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.hoodycode/rules/regular.txt") {
				return Promise.resolve("regular file content")
			}
			if (normalizedPath === "/fake/path/.hoodycode/symlink-target.txt") {
				return Promise.resolve("symlink target content")
			}
			if (normalizedPath === "/fake/path/.hoodycode/rules/symlink-target-dir/subdir_link.txt") {
				return Promise.resolve("regular file content under symlink target dir")
			}
			if (normalizedPath === "/fake/path/.hoodycode/nested-symlink-target.txt") {
		...
		const expectedRegularPath =
			process.platform === "win32"
				? "\\fake\\path\\.hoodycode\\rules\\regular.txt"
				: "/fake/path/.hoodycode/rules/regular.txt"
		const expectedSymlinkPath =
			process.platform === "win32"
				? "\\fake\\path\\.hoodycode\\symlink-target.txt"
				: "/fake/path/.hoodycode/symlink-target.txt"
		const expectedSubdirPath =
			process.platform === "win32"
				? "\\fake\\path\\.hoodycode\\rules\\symlink-target-dir\\subdir_link.txt"
				: "/fake/path/.hoodycode/rules/symlink-target-dir/subdir_link.txt"
		const expectedNestedPath =
			process.platform === "win32"
				? "\\fake\\path\\.hoodycode\\nested-symlink-target.txt"
				: "/fake/path/.hoodycode/nested-symlink-target.txt"
		...
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.hoodycode/rules/link.txt")
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.hoodycode/rules/link_dir")

		// Verify both files were read
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.hoodycode/rules/regular.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.hoodycode/symlink-target.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith(
			"/fake/path/.hoodycode/rules/symlink-target-dir/subdir_link.txt",
			"utf-8",
		)
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.hoodycode/nested-symlink-target.txt", "utf-8")
	})
	beforeEach(() => {
		vi.clearAllMocks()
	})
	...
	it.skipIf(process.platform === "win32")("should correctly format multiple files from directory", async () => {
		...
		// Simulate listing files
		readdirMock.mockResolvedValueOnce([
			{ name: "file1.txt", isFile: () => true, parentPath: "/fake/path/.hoodycode/rules" },
			{ name: "file2.txt", isFile: () => true, parentPath: "/fake/path/.hoodycode/rules" },
			{ name: "file3.txt", isFile: () => true, parentPath: "/fake/path/.hoodycode/rules" },
		] as any)
		...
		statMock.mockImplementation((path) => {
			...
			expect([
				"/fake/path/.hoodycode/rules/file1.txt",
				"/fake/path/.hoodycode/rules/file2.txt",
				"/fake/path/.hoodycode/rules/file3.txt",
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.hoodycode/rules/file3.txt") {
