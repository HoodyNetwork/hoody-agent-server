...
import { hasAnyToggles, loadEnabledRules } from "./hoody"
...
let hasShownNonHoodycodeRulesMessage = false
...
import { ClineRulesToggles } from "../../../shared/cline-rules" // hoodycode_change
...
async function readTextFilesFromDirectory(dirPath: string): Promise<Array<{ filename: string; content: string }>> {
	try {
		...
		// hoodycode_change, must be imported at submodule level because the module is imported in the webview-ui
...
export async function loadRuleFiles(cwd: string): Promise<string> {
	...
	const ruleFiles = [".hoodycoderules", ".roorules", ".clinerules"]

	for (const file of ruleFiles) {
		const content = await safeReadFile(path.join(cwd, file))
		if (content) {
			if (file !== ".hoodycoderules" && vscodeAPI && !hasShownNonHoodycodeRulesMessage) {
				// hoodycode_change: show message to move to .hoodycode/rules/
				vscodeAPI.window.showWarningMessage(
					`Loading non-Hoodycode rules from ${file}, consider moving to .hoodycode/rules/`,
				)
				hasShownNonHoodycodeRulesMessage = true
			} // hoodycode_change end
			return `\n# Rules from ${file}:\n${content}\n`
		}
	}
...
export async function addCustomInstructions(
	...
	cwd: string,
	mode: string,
	// hoodycode_change begin: rule toggles
	options: {
		language?: string
	...
	if (mode) {
		...
		if (modeRules.length > 0) {
			...
			const rooModeRuleFile = `.hoodycoderules-${mode}`
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		if (usedRuleFile.includes(path.join(".hoodycode", `rules-${mode}`))) {
	...
	// hoodycode_change start: rule toggles
