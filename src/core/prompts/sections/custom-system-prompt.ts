...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	// hoodycode_change
	return path.join(cwd, ".hoodycode", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .hoodycode/system-prompt-[mode slug]
...
/**
 * Ensures the .hoodycode directory exists, creating it if necessary
 */
export async function ensureRooDirectory(cwd: string): Promise<void> {
	// hoodycode_change
	const rooDir = path.join(cwd, ".hoodycode")
