...
export function checkExistKey(config: ProviderSettings | undefined) {
	...
	if (
		config.apiProvider &&
		["human-relay", "fake-ai", "claude-code", "qwen-code", "roo", "gemini-cli"].includes(config.apiProvider) // hoodycode_change: add gemini-cli
	) {
		return true
	}
	...
	const hasOtherConfig = [
		...
		config.hoodycodeModel, // hoodycode_change
