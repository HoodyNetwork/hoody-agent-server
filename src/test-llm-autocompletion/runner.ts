...
function checkEnvironment() {
	const provider = process.env.LLM_PROVIDER || "hoodycode"

	if (provider !== "hoodycode") {
		console.error(`\n❌ Error: Only hoodycode provider is supported. Got: ${provider}`)
		process.exit(1)
	}

	if (!process.env.HOODYCODE_API_KEY) {
		console.error(`\n❌ Error: HOODYCODE_API_KEY is not set`)
		console.log("\nPlease create a .env file with your API credentials.")
		console.log("Example: HOODYCODE_API_KEY=your-api-key-here\n")
		process.exit(1)
	}
}
