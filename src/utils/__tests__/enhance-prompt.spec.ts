...
describe("enhancePrompt", () => {
	...
	// hoodycode_change start - updated tests to work with createMessage fallback
	...
	it("handles streaming errors gracefully in fallback mode", async () => {
		...
		;(buildApiHandler as any).mockReturnValue({
			...
			createMessage: vi.fn().mockReturnValue(mockStream), // hoodycode_change
			getModel: vi.fn().mockReturnValue({
				id: "test-model",
				info: {
					maxTokens: 4096,
	...
	// hoodycode_change end - updated tests to work with createMessage fallback
