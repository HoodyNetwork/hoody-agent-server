...
export async function generateTerminalCommand(options: TerminalCommandGeneratorOptions): Promise<void> {
	...
	try {
		...
		if (!activeTerminal) {
			vscode.window.showErrorMessage(t("hoodycode:terminalCommandGenerator.noActiveTerminal"))
			return
		}
...
async function showWarningIfNeeded(context: vscode.ExtensionContext): Promise<boolean> {
	...
	if (!warningAcknowledged) {
		const warningChoice = await vscode.window.showInformationMessage(
			t("hoodycode:terminalCommandGenerator.warningDialog.title"),
			{ modal: true, detail: t("hoodycode:terminalCommandGenerator.warningDialog.message") },
			t("hoodycode:terminalCommandGenerator.warningDialog.okButton"),
			t("hoodycode:terminalCommandGenerator.warningDialog.cancelButton"),
		)

		if (warningChoice !== t("hoodycode:terminalCommandGenerator.warningDialog.okButton")) {
			return false
		}
...
async function getUserInput(): Promise<string | undefined> {
	return await vscode.window.showInputBox({
		prompt: t("hoodycode:terminalCommandGenerator.inputPrompt"),
		placeHolder: t("hoodycode:terminalCommandGenerator.inputPlaceholder"),
		ignoreFocusOut: true,
	})
}
	...
	await vscode.window.withProgress(
		{
			...
			title: t("hoodycode:terminalCommandGenerator.generatingProgress"),
			cancellable: false,
		},
		async () => {
			...
			vscode.window.showInformationMessage(
				t("hoodycode:terminalCommandGenerator.commandGenerated", { command: cleanCommand }),
			)
		},
	)
}
...
function handleError(error: unknown, outputChannel: vscode.OutputChannel, context: string): void {
	...
	if (context === "generateTerminalCommand") {
		vscode.window.showErrorMessage(t("hoodycode:terminalCommandGenerator.generationFailed", { error: errorMessage }))
	} else {
