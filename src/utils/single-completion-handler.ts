...
import { buildApiHandler, SingleCompletionHandler, ApiHandler } from "../api" //hoodycode_change
...
export async function singleCompletionHandler(apiConfiguration: ProviderSettings, promptText: string): Promise<string> {
	...
	if (!("completePrompt" in handler)) {
		// hoodycode_change start - stream responses for handlers without completePrompt
...
// hoodycode_change start - Stream responses using createMessage
...
// hoodycode_change end - streamResponseFromHandler
