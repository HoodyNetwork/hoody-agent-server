// hoodycode_change - new file Support JSON-based launch configurations
 ...
 * Reads .hoodycode/launchConfig.json from the workspace root.
 */
export async function checkAndRunAutoLaunchingTask(context: vscode.ExtensionContext): Promise<void> {
	...
	const configPath = vscode.Uri.joinPath(workspaceFolderUri, ".hoodycode", "launchConfig.json")
	...
	try {
		...
		vscode.commands.executeCommand("hoody-code.newTask", config) // Pass the full config to newTask
	} catch (error) {
