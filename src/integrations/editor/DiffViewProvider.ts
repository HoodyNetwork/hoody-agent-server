...
export const DIFF_VIEW_LABEL_CHANGES = "Original ↔ Hoody Code's Changes"
...
export class DiffViewProvider {
	...
	async open(relPath: string): Promise<void> {
		...
		if (!fileExists) {
			await fileWriteQueue.writeFile(absolutePath, "")
		}
	...
	async saveDirectly(
		...
		// Write the content directly to the file (using queue for concurrency protection)
		await createDirectoriesForFile(absolutePath)
		await fileWriteQueue.writeFile(absolutePath, content, "utf-8")
