...
async function showMacOSNotification(options: NotificationOptions): Promise<void> {
	...
	try {
		...
		// Add Hoody Code logo
		const extensionUri = vscode.extensions.getExtension(`hoodycode.hoody-code`)!.extensionUri
		const iconPath = vscode.Uri.joinPath(extensionUri, "assets", "icons", "ai-agent-dark-64px.png").fsPath
...
"@
    ...
    [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("Hoody Code").Show($toast)
    `
...
export async function showSystemNotification(options: NotificationOptions): Promise<void> {
	try {
		const { title = "Hoody Code", message } = options
