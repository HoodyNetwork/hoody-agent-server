...
type ClaudeCodeOptions = {
	systemPrompt: string
	systemPromptFile?: string // hoodycode_change
...
async function generateTempSystemPrompt(options: ClaudeCodeOptions): Promise<string | undefined> {
	...
	const tempFilePath = path.join(os.tmpdir(), `hoodycode-system-prompt-${uniqueId}.txt`)
...
export async function* runClaudeCode(
	...
	const systemPromptFile = await generateTempSystemPrompt(options) // hoodycode_change
	const claudePath = options.path || "claude"
	let process

	try {
		process = runProcess({ ...options, systemPromptFile }) // hoodycode_change
	} catch (error: any) {
...
function runProcess({
	...
	// const isWindows = os.platform() === "win32" hoodycode_change
	...
	const stdinData = JSON.stringify(messages) // hoodycode_change
