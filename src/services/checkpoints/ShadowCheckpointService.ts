...
function showWarning(message: string) {
	if (warningsShown.size < 5 && !warningsShown.has(message)) {
		vscode.window.showWarningMessage(message, t("hoodycode:checkpoints.dismissWarning"))
...
export abstract class ShadowCheckpointService extends EventEmitter {
	...
	constructor(taskId: string, checkpointsDir: string, workspaceDir: string, log: (message: string) => void) {
		...
		if (protectedPaths.includes(workspaceDir)) {
			showWarning(t("hoodycode:checkpoints.protectedPaths", { workspaceDir })) // hoodycode_change
	...
	public async initShadowGit(onInit?: () => Promise<void>) {
		...
		if (nestedGitPath) {
			...
			showWarning(t("hoodycode:checkpoints.nestedGitRepos", { path: relativePath })) // hoodycode_change
		...
		if (await fileExistsAtPath(this.dotGitDir)) {
			...
			await git.addConfig("user.name", "Hoody Code")
	...
	private async stageAll(git: SimpleGit) {
		try {
			...
			reportError(`${this.constructor.name}#stageAll`, error) // hoodycode_change
		}
	}
	...
	private async getNestedGitRepository(): Promise<string | null> {
		try {
			...
			reportError(`${this.constructor.name}#hasNestedGitRepositories`, error) // hoodycode_change
	...
	private async getShadowGitConfigWorktree(git: SimpleGit) {
		if (!this.shadowGitConfigWorktree) {
			try {
				...
				reportError(`${this.constructor.name}#getShadowGitConfigWorktree`, error) // hoodycode_change
			}
		}
	...
	public async getDiff({ from, to }: { from?: string; to?: string }): Promise<CheckpointDiff[]> {
		...
		for (const file of files) {
			...
			const before = await this.git.show([`${from}:${relPath}`]).catch((err) => {
				reportError(`[${this.constructor.name}#getDiff:git.show:before`, err) // hoodycode_change
				return ""
			})

			const after = to
				? await this.git.show([`${to}:${relPath}`]).catch((err) => {
						reportError(`[${this.constructor.name}#getDiff:git.show:after`, err) // hoodycode_change
						return ""
					})
				: await fs.readFile(absPath, "utf8").catch((err) => {
						reportError(`[${this.constructor.name}#getDiff:readFile`, err) // hoodycode_change
						return ""
					})
	...
	public static async deleteBranch(git: SimpleGit, branchName: string) {
		...
		if (currentBranch === branchName) {
			...
			try {
				...
				reportError(`${this.constructor.name}#deleteBranch`, error) // hoodycode_change
