...
const HOODYCODE_DEFAULT_MODEL = "mistralai/codestral-2508"
const MISTRAL_DEFAULT_MODEL = "codestral-latest"

const SUPPORTED_DEFAULT_PROVIDERS = ["mistral", "hoodycode", "openrouter"]
...
export class GhostModel {
	...
	public async reload(settings: GhostServiceSettings, providerSettingsManager: ProviderSettingsManager) {
		...
		if (selectedProfile) {
			...
			let modelDefinition = {}
			if (profileProvider === "hoodycode") {
				modelDefinition = {
					hoodycodeModel: HOODYCODE_DEFAULT_MODEL,
				}
			} else if (profileProvider === "openrouter") {
				modelDefinition = {
					openRouterModelId: HOODYCODE_DEFAULT_MODEL,
				}
