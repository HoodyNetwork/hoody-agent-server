...
export class GhostProvider {
	...
	public async promptCodeSuggestion() {
		...
		const userInput = await vscode.window.showInputBox({
			prompt: t("hoodycode:ghost.input.title"),
			placeHolder: t("hoodycode:ghost.input.placeholder"),
		})
		if (!userInput) {
			return
		}
	...
	private async updateGlobalContext() {
		...
		await vscode.commands.executeCommand("setContext", "hoodycode.ghost.hasSuggestions", hasSuggestions)
		await vscode.commands.executeCommand("setContext", "hoodycode.ghost.isProcessing", this.isProcessing)
		await vscode.commands.executeCommand(
			"setContext",
			"hoodycode.ghost.enableQuickInlineTaskKeybinding",
			this.settings?.enableQuickInlineTaskKeybinding || false,
		)
		await vscode.commands.executeCommand(
			"setContext",
			"hoodycode.ghost.enableSmartInlineTaskKeybinding",
	...
	public async showIncompatibilityExtensionPopup() {
		const message = t("hoodycode:ghost.incompatibilityExtensionPopup.message")
		const disableCopilot = t("hoodycode:ghost.incompatibilityExtensionPopup.disableCopilot")
		const disableInlineAssist = t("hoodycode:ghost.incompatibilityExtensionPopup.disableInlineAssist")
		...
		if (response === disableCopilot) {
			...
			await vscode.commands.executeCommand<any>("hoody-code.ghost.disable")
		}
	}
