...
export class TerminalWelcomeService {
	...
	private handleTerminalOpened(terminal: vscode.Terminal): void {
		...
		this.tipShownThisSession = true // hoodycode_change: Mark as shown for this session
		setTimeout(() => this.showWelcomeMessage(terminal), 500)
	}

	private async showWelcomeMessage(terminal: vscode.Terminal): Promise<void> {
		const shortcut = await getKeybindingForCommand("hoody-code.generateTerminalCommand")
		const message = t("hoodycode:terminalCommandGenerator.tipMessage", { shortcut })
