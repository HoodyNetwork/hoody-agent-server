...
export class SimpleInstaller {
	...
	private async installMode(
		...
		try {
			...
			if (error.code === "ENOENT") {
				...
				const fileName = target === "project" ? ".hoodycodemodes" : "custom-modes.yaml"
				throw new Error(
	...
	private async installMcp(
		...
		try {
			...
			if (error.code === "ENOENT") {
				...
				const fileName = target === "project" ? ".hoodycode/mcp.json" : "mcp-settings.json"
				throw new Error(
	...
	private async getModeFilePath(target: "project" | "global"): Promise<string> {
		if (target === "project") {
			...
			return path.join(workspaceFolder.uri.fsPath, ".hoodycodemodes")
		} else {
	...
	private async getMcpFilePath(target: "project" | "global"): Promise<string> {
		if (target === "project") {
			...
			return path.join(workspaceFolder.uri.fsPath, ".hoodycode", "mcp.json")
		} else {
			const globalSettingsPath = await ensureSettingsDirectoryExists(this.context)
