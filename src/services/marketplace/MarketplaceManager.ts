...
export class MarketplaceManager {
	...
	private async checkProjectInstallations(metadata: Record<string, { type: string }>): Promise<void> {
		try {
			...
			const projectModesPath = path.join(workspaceFolder.uri.fsPath, ".hoodycodemodes")
			try {
			...
			const projectMcpPath = path.join(workspaceFolder.uri.fsPath, ".hoodycode", "mcp.json")
			try {
