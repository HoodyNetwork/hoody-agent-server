...
import fsSync from "fs" // hoodycode_change
...
export function getGlobalRooDirectory(): string {
	...
	return path.join(homeDir, ".hoodycode") // hoodycode_change
}
...
export function getProjectRooDirectoryForCwd(cwd: string): string {
	...
	const hoodyDir = path.join(cwd, ".hoodycode")
	const rooDir = path.join(cwd, ".roo")
	if (fsSync.existsSync(rooDir) && !fsSync.existsSync(hoodyDir)) {
		return rooDir
	}
	return hoodyDir
