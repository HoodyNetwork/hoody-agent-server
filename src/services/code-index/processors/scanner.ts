...
export class DirectoryScanner implements IDirectoryScanner {
	private _cancelled = false // hoodycode_change
	...
	public async scanDirectory(
				...
				try {
					...
					if (this.embedder && this.qdrantClient && blocks.length > 0) {
						...
						for (const block of blocks) {
							if (this._cancelled) break // hoodycode_change
		...
		// hoodycode_change: add !this._cancelled
	...
	private async processBatch(
		...
		while (attempts < MAX_BATCH_RETRIES && !success) {
			...
			try {
				// --- Deletion Step ---
				console.debug("[DirectoryScanner] Starting deletion step for modified files") // hoodycode_change
				...
				if (this._cancelled) return // hoodycode_change

				console.debug(`[DirectoryScanner] Creating embeddings for ${batchTexts.length} texts`) // hoodycode_change

				const { embeddings } = await this.embedder.createEmbeddings(batchTexts)
				console.debug(`[DirectoryScanner] Successfully created ${embeddings.length} embeddings`) // hoodycode_change

				// Prepare points for Qdrant
				console.debug("[DirectoryScanner] Preparing points for Qdrant upsert") // hoodycode_change
				...
				console.debug(`[DirectoryScanner] Prepared ${points.length} points for Qdrant`) // hoodycode_change

				// Upsert points to Qdrant
				if (this._cancelled) return // hoodycode_change

				console.debug("[DirectoryScanner] Starting Qdrant upsert") // hoodycode_change

				await this.qdrantClient.upsertPoints(points)
				console.debug("[DirectoryScanner] Completed Qdrant upsert") // hoodycode_change
				onBlocksIndexed?.(batchBlocks.length)

				// Update hashes for successfully processed files in this batch
				console.debug("[DirectoryScanner] Updating file hashes in cache") // hoodycode_change
				for (const fileInfo of batchFileInfos) {
					await this.cacheManager.updateHash(fileInfo.filePath, fileInfo.fileHash)
				}
				console.debug("[DirectoryScanner] Completed updating file hashes in cache") // hoodycode_change
				...
				if (attempts < MAX_BATCH_RETRIES) {
					...
					console.debug(`[DirectoryScanner] Retrying batch in ${delay}ms`) // hoodycode_change
